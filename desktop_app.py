#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""跨平台启动器：桌面托盘模式（Windows / Linux 桌面）与无头服务模式（--serve）。"""
...
from utils.updater import start_update_check, _has_desktop
...
def _show_error(message: str) -> None:
    ...
    # 桌面环境才弹原生窗；无桌面时仅打印（避免 Windows 专有 API 在 Linux 崩溃）
    if _has_desktop():
        try:
            from utils.updater import _platform_popup

            _platform_popup(message, "BiliLearn 错误", kind="error")
        except Exception:
            pass
...
def main() -> int:
    ...
    if _has_desktop():
        ...
        # 无桌面环境（服务器 / 无显示器）：以无头服务模式保持运行
        print(f"BiliLearn Web 无头服务已启动：{URL}  （按 Ctrl+C 停止）")
        try:
