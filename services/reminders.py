...
def list_reminders(data_dir=None) -> list[dict]:
    """Return local reminders newest first for the web task workspace."""
    return sorted(_read(data_dir), key=lambda row: str(row.get("due_at") or ""))


def create_reminder(content: str, due_at: str, *, owner_uid: str = "manual", data_dir=None) -> dict:
    try:
        target = datetime.fromisoformat(str(due_at).strip())
    except ValueError as exc:
        raise ValueError("Reminder time must use YYYY-MM-DDTHH:MM") from exc
    text = str(content or "").strip()
    if not text:
        raise ValueError("Reminder content cannot be empty")
    row = {
        "id": uuid.uuid4().hex[:12], "owner_uid": str(owner_uid), "content": text[:240],
        "due_at": target.isoformat(timespec="minutes"), "created_at": datetime.now().isoformat(timespec="seconds"),
        "status": "pending", "delivered_at": "",
    }
    with _LOCK:
        rows = _read(data_dir)
...
def remove_reminder(reminder_id: str, data_dir=None) -> bool:
    with _LOCK:
        rows = _read(data_dir)
        kept = [row for row in rows if str(row.get("id")) != str(reminder_id)]
