...
def export_text(content, filename, fmt="txt"):
    """导出为 txt/json/html/md 格式。"""
    from pathlib import Path
    ...
    if fmt == "json":
        ...
        filepath = export_dir / f"{safe_name}_{timestamp}.md"
        filepath.write_text(content, encoding="utf-8")
    else:  # txt
        filepath = export_dir / f"{safe_name}_{timestamp}.txt"
        filepath.write_text(content, encoding="utf-8")
