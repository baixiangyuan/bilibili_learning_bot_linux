...
【互动能力（可选，需要时主动使用）】
- 遇到优质视频，可以用 like_video / coin_video / favorite_video 点赞/投币/收藏
- 用 triple_action 一键执行"伪三连"（点赞+投币+收藏），推荐在遇到高分优质视频时使用
- 用 follow_up 关注你反复学习或质量很高的UP主，用 do_share 转发分享
- 这些互动会受用户的功能开关控制（可能被关闭），失败也不影响学习主流程，无需向用户报错"""
...
TOOLS_DEEP_DIVE = [
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "bvid": {"type": "string", "description": "视频BV号"},
                    "amount": {"type": "integer", "description": "投币数量(1-2)", "default": 1},
                },
                "required": ["bvid"],
            },
        },
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "uid": {"type": "integer", "description": "UP主UID"},
                },
                "required": ["uid"],
            },
        },
    },
    {
        ...
        "function": {
            "name": "get_video_info",
            "description": "获取B站视频详细信息(标题/UP主/播放量/封面等)",
            "parameters": {
                "type": "object",
                "properties": {
                    "bvid": {"type": "string", "description": "视频BV号"},
    ...
    },
    {
        ...
        "function": {
            "name": "triple_action",
            "description": "对视频执行三连（伪三连=点赞+投币+收藏），一次性完成互动",
            "parameters": {
                "type": "object",
                "properties": {
                    "bvid": {"type": "string", "description": "视频BV号"},
...
def _get_credential():
    ...
    try:
        ...
        return getattr(g, "credential", None)
    except Exception:
        return None

def _safe_int(value, default: int) -> int:
    """安全转 int：AI 传非法参数时回退默认值，避免整个 agent 循环崩溃。"""
    try:
        return int(value)
...
def _make_tool_handler(session: LearningAgentSession, on_finalize: Callable | None = None):
    ...
    async def handler(tool_name: str, tool_args: dict) -> str:
        if tool_name == "search_bilibili":
            ...
            count = _safe_int(tool_args.get("count", 8), 8)
            ...
            query = tool_args.get("query", "")
            count = _safe_int(tool_args.get("count", 5), 5)
            ...
            question_count = _safe_int(tool_args.get("question_count", 0), 0)
            ...
            bvid = tool_args.get("bvid", "")
            try:
                from core.platform_actions import liking_enabled
                if not liking_enabled():
                    return json.dumps({"ok": False, "message": "点赞功能已关闭"}, ensure_ascii=False)
                from bilibili_api import video as bili_video_mod
                v = bili_video_mod.Video(bvid=bvid, credential=getattr(session, "credential", None) or _get_credential())
                await v.like()
                return json.dumps({"ok": True, "message": f"已点赞视频 {bvid}"}, ensure_ascii=False)
            except Exception as e:
                return json.dumps({"ok": False, "message": str(e)}, ensure_ascii=False)

        elif tool_name == "coin_video":
            bvid = tool_args.get("bvid", "")
            amount = _safe_int(tool_args.get("amount", 1), 1)
            try:
                from core.platform_actions import coining_enabled
                if not coining_enabled():
                    return json.dumps({"ok": False, "message": "投币功能已关闭"}, ensure_ascii=False)
                from bilibili_api import video as bili_video_mod
                v = bili_video_mod.Video(bvid=bvid, credential=getattr(session, "credential", None) or _get_credential())
                await v.pay_coin(num=amount, like=True)
                return json.dumps({"ok": True, "message": f"已投币视频 {bvid}"}, ensure_ascii=False)
            except Exception as e:
                return json.dumps({"ok": False, "message": str(e)}, ensure_ascii=False)
            ...
            try:
                ...
                from bilibili_api import video as bili_video_mod, favorite_list
                v = bili_video_mod.Video(bvid=bvid, credential=getattr(session, "credential", None) or _get_credential())
                _cred = getattr(session, "credential", None) or _get_credential()
                if _cred:
                    fav_data = await favorite_list.get_video_favorite_list(uid=_cred.dedeuserid, credential=_cred)
                    if fav_data and fav_data.get('list'):
                        folder_id = fav_data['list'][0]['id']
                        await v.set_favorite(add_media_ids=[folder_id])
                return json.dumps({"ok": True, "message": f"已收藏视频 {bvid}"}, ensure_ascii=False)
            except Exception as e:
                return json.dumps({"ok": False, "message": str(e)}, ensure_ascii=False)

        elif tool_name == "follow_up":
            uid = _safe_int(tool_args.get("uid", 0), 0)
            try:
                from core.platform_actions import following_enabled
                if not following_enabled():
                    return json.dumps({"ok": False, "message": "关注功能已关闭"}, ensure_ascii=False)
                from bilibili_api import user as bili_user_mod
                u = bili_user_mod.User(uid=uid, credential=getattr(session, "credential", None) or _get_credential())
                await u.modify_relation(bili_user_mod.Relation.SUBSCRIBE)
                return json.dumps({"ok": True, "message": f"已关注UP主 {uid}"}, ensure_ascii=False)
            except Exception as e:
                return json.dumps({"ok": False, "message": str(e)}, ensure_ascii=False)
        ...
        elif tool_name == "do_share":
            bvid = tool_args.get("bvid", "")
            try:
                return json.dumps({"ok": True, "message": f"已生成分享链接: https://www.bilibili.com/video/{bvid}",
                                   "share_url": f"https://www.bilibili.com/video/{bvid}"}, ensure_ascii=False)
            except Exception as e:
                return json.dumps({"ok": False, "message": str(e)}, ensure_ascii=False)

        elif tool_name == "triple_action":
            bvid = tool_args.get("bvid", "")
            amount = _safe_int(tool_args.get("amount", 1), 1)
            results = {}
            try:
                from core.platform_actions import liking_enabled, coining_enabled, favoriting_enabled
                from bilibili_api import video as bili_video_mod, favorite_list
                v = bili_video_mod.Video(bvid=bvid, credential=getattr(session, "credential", None) or _get_credential())
                if liking_enabled():
                    try:
                ...
                if coining_enabled():
                    ...
                    results["coin"] = "跳过(开关关闭)"
                if favoriting_enabled():
                    try:
                        _cred = getattr(session, "credential", None) or _get_credential()
                        if _cred:
                            fav_data = await favorite_list.get_video_favorite_list(uid=_cred.dedeuserid, credential=_cred)
                            if fav_data and fav_data.get('list'):
                                ...
                                results["favorite"] = "无收藏夹"
                    except Exception as e:
                        results["favorite"] = f"失败: {e}"
                else:
                    results["favorite"] = "跳过(开关关闭)"
                return json.dumps({"ok": True, "message": "伪三连执行完成", "results": results}, ensure_ascii=False)
            except Exception as e:
                return json.dumps({"ok": False, "message": str(e), "results": results}, ensure_ascii=False)
