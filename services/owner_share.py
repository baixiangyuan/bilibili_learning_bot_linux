...
TEST_SHARE_SYSTEM_PROMPT = (
    ...
    "Do not use Unicode emoji. If a little emotion is useful, use at most one supplied Bilibili bracket emote such as [doge]. "
...
class OwnerShareService:
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "willingness": _number(cfg.get("willingness", cfg.get("probability")), 0.35, 0, 1),
            "cooldown_minutes": int(_number(cfg.get("cooldown_minutes"), 30, 0, 1440)),
            "match_tags": cfg.get("match_tags", []) if isinstance(cfg.get("match_tags", []), list) else [],
    ...
    def _check(self, cfg: dict[str, Any], state: dict[str, Any], *, bvid: str, title: str,
        ...
        if not cfg.get("share_learned", True) and not fun_enabled:
            return "当前分享类型未匹配"
        # willingness is the user-facing control; probability remains a legacy alias.
        share_probability = _number(cfg.get("willingness", cfg.get("probability")), 0.35, 0, 1)
