...
def _compat_resp(data: dict):
    """把 OpenAI 兼容 JSON 响应构造成带 .choices/.message/.tool_calls/.model_dump 的轻量对象（纯 httpx，不依赖 openai 库）。"""
    ...
    class _ToolCall:
        def __init__(self, d):
            ...
            self.function = _ToolFn(d.get("function", {}) or {})
...
async def _call_ai_via_httpx(
    ...
    *,
    ...
    # 构造兼容 OpenAI 响应对象（纯 httpx，不依赖 openai 库）
    return _compat_resp(data)
    ...
    for attempt in range(max_attempts):
        for backend_name, backend_fn in backends:
            try:
                ...
                last_error = e
                if "http 402" in str(e).lower() or "insufficient balance" in str(e).lower() or "insufficient quota" in str(e).lower() or "quota exceeded" in str(e).lower():
                    try:
                        from services.quota_alerts import alert_billing_error_async
                        alert_billing_error_async(str(e))
                    except Exception:
                        pass
...
async def call_ai_with_tools(
    ...
    *,
    ...
    base_url = live.get("base_url", "")
    if not base_url or "://" not in str(base_url):
        raise RuntimeError(f"API地址无效: '{base_url}'，请在配置菜单中设置有效的API地址")
    _url = f"{base_url.rstrip('/')}/chat/completions"
    _proxy_url = ""
    try:
        from services.proxy_config import get_proxy_url
        _proxy_url = get_proxy_url()
