...
def _safe_logging_shutdown(*args, **kwargs):
    try:
        _orig_logging_shutdown()
...
from cli.app import (
    ...
    show_watch_later_menu, show_reminders_menu,
    show_interaction_switches_menu, show_watch_history_menu,
)
...
def main():
    ...
    while True:
        ...
        _first_menu = False
        show_main_menu()
        choice = input(f"{Fore.CYAN}请输入选项 (0-7/9/A-Z/ALL/AR/BN/MM/RT/WB/WL/RM/SW/WH): {Style.RESET_ALL}").strip()
    ...
    {Style.RESET_ALL}""")
        ...
        # Keep the implementation in cli.app for internal testing only.
            ...
            try:
                ...
                traceback.print_exc()
        elif choice.lower() == "wl":
            try:
                show_watch_later_menu()
            except Exception as e:
                print(f"{Fore.RED}[ERROR] 稍后再看管理异常: {e}{Style.RESET_ALL}")
                traceback.print_exc()
        elif choice.lower() == "rm":
            try:
                show_reminders_menu()
            except Exception as e:
                print(f"{Fore.RED}[ERROR] 待办管理异常: {e}{Style.RESET_ALL}")
                traceback.print_exc()
        elif choice.lower() == "sw":
            try:
                show_interaction_switches_menu()
            except Exception as e:
                print(f"{Fore.RED}[ERROR] 功能开关异常: {e}{Style.RESET_ALL}")
                traceback.print_exc()
        elif choice.lower() == "wh":
            try:
                show_watch_history_menu()
            except Exception as e:
                print(f"{Fore.RED}[ERROR] 观看历史异常: {e}{Style.RESET_ALL}")
                traceback.print_exc()
