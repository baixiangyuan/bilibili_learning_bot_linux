...
class PrivateMessageManager:
    ...
    def __init__(self, credential, uid, since_ts=0, previous_seen_at=""):
        ...
        _cfg_bl = [str(x).strip() for x in config.get("private_message", {}).get("blacklist_uids", []) if str(x).strip()]
        # 默认拉黑系统机器人（哔哩哔哩智能机），用户可在配置中移除
        if "12076317" not in _cfg_bl:
            _cfg_bl.append("12076317")
        self.pm_blacklist = _cfg_bl
        self.pm_whitelist = [str(x).strip() for x in config.get("private_message", {}).get("whitelist_uids", []) if str(x).strip()]
    ...
    async def process_new_messages(self, max_replies=None, auto_reply=None):
        ...
        for msg in messages[:reply_limit]:
            try:
                ...
                _bl = getattr(self, "pm_blacklist", [])
                _wl = getattr(self, "pm_whitelist", [])
