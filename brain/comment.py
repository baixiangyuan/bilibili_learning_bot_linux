...
class CommentInteractionManager:
    ...
    async def reply_to_comment(self, bili_client, comment_data, ai_response, *, is_at_mention=False):
        ...
        try:
            ...
            # 回复成功后一键三连（受功能开关控制；动态评论无视频可三连，跳过）
            try:
                from core.platform_actions import liking_enabled, coining_enabled, favoriting_enabled
                if str(comment_data.get("resource_type") or "video").lower() == "dynamic":
                    log("[评论三连] 动态评论跳过三连（无关联视频）", "COMMENT")
                    return True
                bvid = str(comment_data.get("bvid") or "")
                aid = comment_data.get("aid")
                # 支持 aid 兜底：部分来源（通知/艾特）只带 aid 不带 bvid
                from bilibili_api import video as bili_video_mod
                v = None
                try:
                    if bvid:
                        v = bili_video_mod.Video(bvid=bvid, credential=self.credential)
                    elif aid and str(aid).isdigit():
                        v = bili_video_mod.Video(aid=int(aid), credential=self.credential)
                except Exception:
                    v = None
                if v is not None:
                    if liking_enabled():
                        try:
                            await v.like()
                            log(f"[评论三连] 已点赞 {bvid}", "SUCCESS")
                        except Exception as _e:
                            log(f"[评论三连] 点赞失败: {_e}", "WARN")
                    if coining_enabled():
                        try:
                            await v.pay_coin(num=1, like=True)
                            log(f"[评论三连] 已投币 {bvid}", "SUCCESS")
                        except Exception as _e:
                            log(f"[评论三连] 投币失败: {_e}", "WARN")
                    if favoriting_enabled():
                        try:
                            ...
                            if fav_list_data and fav_list_data.get('list'):
                                ...
                                log(f"[评论三连] 已收藏 {bvid}", "SUCCESS")
                        except Exception as _e:
                            log(f"[评论三连] 收藏失败: {_e}", "WARN")
            except Exception:
                pass
            return True
