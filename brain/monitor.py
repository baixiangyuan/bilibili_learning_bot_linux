...
class MonitorBot:
    ...
    async def _generate_at_reply(self, notification):
        ...
        is_content_request = self._asks_about_video(notification.get("content", ""))
        try:
            ...
            if bvid and self.comment_mgr and getattr(self.comment_mgr, "toolbox", None):
                ...
                # 如果用户要求总结/讲解视频内容，真正获取字幕
                if is_content_request:
                    try:
                        from api.subtitles import fetch_bilibili_subtitles
                        sub_result = await fetch_bilibili_subtitles(bvid, self.bili.credential)
                        # fetch_bilibili_subtitles 返回 (success, content, video_desc, ai_verified)
                        if isinstance(sub_result, tuple) and len(sub_result) >= 2:
                            subtitle_text = sub_result[1] if sub_result[0] else ""
                        elif isinstance(sub_result, str):
                            subtitle_text = sub_result
                        else:
                            subtitle_text = ""
                        if subtitle_text:
                            evidence["subtitles"] = subtitle_text[:5000]
                            evidence["status"] = "with_subtitles"
                            log(f"[监听][@] 已获取字幕({len(subtitle_text)}字)用于回复 @{notification.get('user', '未知')} (UID:{notification.get('user_id', '-')})", "INFO")
                        else:
                            evidence["subtitles"] = ""
                            evidence["status"] = "no_subtitles"
                    except Exception as sub_exc:
                        log(f"[监听][@] 字幕获取失败: {sub_exc}", "WARN")
                        evidence["subtitles"] = ""
                        evidence["status"] = "subtitle_fetch_failed"
        ...
        if is_content_request and evidence.get("subtitles"):
            prompt = (
                "Reply to this Bilibili @ mention. The user is asking about the video content. "
                "Use the subtitles and metadata below to provide a helpful summary or answer. "
                "Reply in the language used by the commenter. Be concise but informative.\n\n"
                f"Commenter: {notification.get('user')}\n"
                f"Comment: {notification.get('content')}\n"
                f"Video Title: {evidence.get('title', 'unknown')}\n"
                f"Subtitles: {evidence.get('subtitles', '')[:4000]}\n"
                f"Description: {evidence.get('description', '')[:500]}"
            )
        else:
            prompt = (
        ...
        reply = await call_ai(
            [{"role": "system", "content": "You are a concise, factual Bilibili AI comment assistant. Do not use Unicode emoji; use at most one Bilibili bracket emote such as [doge] only when useful."},
        ...
        try:
            ...
            for notification in (item for item in items if item["id"] not in self._processed_at_ids):
                ...
                log(
                    ...
                    f"from=@{notification.get('user', '未知')} (UID:{notification.get('user_id', '-')})",
                    "MENTION",
                )
                ...
                if reply and self.cfg.get("auto_reply", True) and notification.get("aid") and notification.get("comment_id"):
                    ...
                    reply_target = {
                        ...
                        "resource_type": "dynamic" if str(notification.get("business") or "").lower() in {"动态", "dynamic", "dyn"} else "video",
                ...
                if not attempted or sent or terminal:
                    ...
                    if attempts >= 3:
                        self._mark_at_processed(notification["id"])
                        processed += 1
                        log(f"mention delivery abandoned after 3 attempts: @{notification.get('user', '未知')} (UID:{notification.get('user_id', '-')})", "WARN")
