...
def _write_activity_state(state, detail="", bvid="", title=""):
    """写入实时活动状态文件，供 Web 面板读取。"""
    try:
        from datetime import datetime
        import json
        try:
            ...
            state_file = Path(__file__).resolve().parent.parent / "Data" / "bot_activity_state.json"
        state_data = {
            "state": state, "detail": detail, "bvid": bvid, "title": title,
...
async def select_candidate_video(brain, items):
    ...
    filtered_pool = []
    for item in pool:
        pubdate = item.get("pubdate") or item.get("publish_date") or 0
...
class BrainLoopMixin:
    ...
    async def run(self):
        ...
        elif not config.get("speed", {}).get("cookie_preheat", True):
            log("⚡ Cookie预热已关闭", "INFO")
        else:
                ...
                if revisit_target:
                    ...
                    if up_browse_target and up_browse_target.get("bvid"):
                        ...
                        if rec_target and rec_target.get("bvid"):
                            ...
                            log("正在刷新推荐流...", "SCAN")
                            _write_activity_state("刷新推荐流", "正在获取下一批候选视频")
