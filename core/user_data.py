...
def _configured_user_data_dir() -> Path:
    """Read the persistent storage root chosen from the web panel.

    The environment variable still wins for portable/EXE launches. The small
    pointer file lives in the default profile and contains no credentials.
    """
    ...
    try:
        ...
        if target:
            ...
            if candidate != candidate.anchor and len(str(candidate)) > 3:
                return candidate
...
USER_DATA_DIR = Path(os.getenv("BILI_USER_DATA_DIR", "")).expanduser() if _EXPLICIT_USER_DATA_DIR else _configured_user_data_dir()
...
KNOWLEDGE_BASE_DIR = _ARTIFACT_ROOT / "KnowledgeBase"
HIGHLIGHTS_DIR = _ARTIFACT_ROOT / "highlights"
HTML_EXPORTS_DIR = _ARTIFACT_ROOT / "html_exports"
MINDMAPS_DIR = _ARTIFACT_ROOT / "MindMaps"
WORD_DIR = _ARTIFACT_ROOT / "Word"
...
def ensure_user_data_dir() -> Path:
    ...
    for name, project_dir in _PROJECT_DIRECTORIES.items():
        _copy_missing(USER_DATA_DIR / name, project_dir)
    # 迁移：把项目根的所有旧目录复制到用户数据目录（不覆盖已有文件）
    for _dir_name in ["KnowledgeBase", "highlights", "html_exports", "MindMaps", "Word"]:
        old_d = PROJECT_DIR / _dir_name
        new_d = USER_DATA_DIR / _dir_name
        if old_d.exists() and old_d.resolve() != new_d.resolve():
            new_d.mkdir(parents=True, exist_ok=True)
            import shutil as _shutil
            for item in old_d.rglob("*"):
                rel = item.relative_to(old_d)
                target = new_d / rel
                if item.is_dir():
                    target.mkdir(parents=True, exist_ok=True)
                elif not target.exists():
