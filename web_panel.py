...
_bot_exit_alert_lock = threading.Lock()
_bot_exit_alert_shown = False
_ai_error_notify_lock = threading.Lock()
_ai_error_notify_last = 0.0
_ai_error_notify_last_msg = ""
...
def _watch_history_cards() -> list[dict]:
    ...
    global _watch_history_card_cache
    history_path, metadata_path = Path(DATA_DIR) / "history_videos.json", _watch_history_metadata_path()
    try:
        signature = (history_path.stat().st_mtime_ns if history_path.exists() else 0,
                     metadata_path.stat().st_mtime_ns if metadata_path.exists() else 0)
    except OSError:
        signature = (0, 0)
    if _watch_history_card_cache and _watch_history_card_cache[0] == signature:
        return [dict(card) for card in _watch_history_card_cache[1]]
    source = read_json(history_path, {})
    ...
    archived_bvids = _archived_bvids()
    ...
    for bvid, item in grouped.items():
        ...
        archived = bvid.upper() in archived_bvids
        cards.append({
            "bvid": bvid,
    ...
    ordered = sorted(cards, key=lambda card: card["watched_at"], reverse=True)
    _watch_history_card_cache = (signature, ordered)
    return [dict(card) for card in ordered]
...
def _is_critical_ai_failure(text: str) -> bool:
    """Detect AI-provider failures that should stop the bot and alert the user.

    Covers billing errors (HTTP 402 / balance / quota), invalid keys (401),
    rate limiting (429), upstream 5xx, timeouts, connection errors, and any
    explicit AI error marker printed by the child.
    """
    normalized = str(text or "").lower()
    # 显式 AI 错误标记
    ai_markers = (
        "fatal_ai_failure", "ai_error", "llm_error", "ai failed", "llm failed",
        "ai调用失败", "ai 调用失败", "ai请求失败", "ai 请求失败", "模型调用失败",
        "模型请求失败", "大模型错误",
    )
    for marker in ai_markers:
        if marker in normalized:
            return True
    # 计费/配额类错误：HTTP 402 + 余额词，无需 AI 上下文直接判定
    billing_hints = (
        "insufficient balance", "insufficient quota", "quota exceeded", "余额不足", "配额",
    )
    if "http 402" in normalized:
        return True
    # AI 上下文提示词（错误行同时包含这些词时视为 AI 故障）
    ai_context = ("ai", "llm", "openai", "模型", "api key", "apikey", "大模型", "chatgpt", "gpt", "claude", "提示词")
    has_ai_context = any(token in normalized for token in ai_context)
    if not has_ai_context:
        return False
    # 其他错误需要 AI 上下文才判定（避免误伤 B 站网络/普通日志）
    other_hints = (
        "http 401", "http 403", "http 404", "http 429",
        "http 500", "http 502", "http 503", "http 504",
        "invalid api key", "authentication", "unauthorized", "unauthorised",
        "rate limit", "rate_limit", "限流", "频繁",
        "timeout", "timed out", "超时",
        "connection", "connect ", "连接失败", "无法连接", "网络错误",
        "upstream", "provider error", "server error", "服务端错误",
        "context length", "context_length",
    )
    return any(hint in normalized for hint in other_hints)


def _handle_critical_ai_failure(source_line: str) -> bool:
    """Stop the child before a billing failure can trigger local fallback actions.

    Returns True when this call actually handled a new fatal failure (and already
    notified the user); False when the failure was already handled previously.
    """
    global _critical_ai_failure_handled, bot_last_error, _bot_exit_alert_shown
    with _critical_ai_failure_lock:
        if _critical_ai_failure_handled:
            return False
        _critical_ai_failure_handled = True
    detail = (str(source_line or "") or "AI account balance or quota is unavailable").strip()[:200]
    if not detail.lower().startswith("http") and "AI " not in detail and "ai " not in detail.lower():
        detail = f"AI 服务异常：{detail}"
    bot_last_error = detail
    try:
        from services.quota_alerts import alert_billing_error_async
        alert_billing_error_async(detail)
    except Exception:
        pass
    log_line(f"[ALERT] {detail} 机器人将自动停止。")
    from utils.desktop_notifications import enqueue as enqueue_desktop_notification
    enqueue_desktop_notification(DATA_DIR, "BiliLearn AI 错误", f"{detail}，机器人已自动停止。")
    tray = _system_tray
    if tray is not None:
        tray.notify("BiliLearn AI 错误", f"{detail}，机器人已自动停止。")
    _bot_exit_alert_shown = True
    threading.Thread(
        ...
        for line in iter(pipe.readline, ""):
            ...
            if text:
                if _is_critical_ai_failure(text):
                    if not _handle_critical_ai_failure(text):
                        # 非致命/已处理过的 AI 错误：仅弹系统通知，不停止机器人
                        _notify_ai_error(text)
...
def _maybe_alert_bot_exit(exit_code) -> None:
    """Alert the user when the bot exited abnormally and the recent log shows an AI failure."""
    global _bot_exit_alert_shown
    with _bot_exit_alert_lock:
        if _bot_exit_alert_shown:
            return
        if _critical_ai_failure_handled:
            _bot_exit_alert_shown = True
            return
        ...
        # 扫描最近运行日志，找 AI 错误行
        try:
            tail = _read_runtime_log(BOT_RUNTIME_LOG_FILE, [], limit=200)
        except Exception:
            tail = []
        if not tail:
            return
        ...
        for line in reversed(tail):
            if _is_critical_ai_failure(line):
                hit = line
                break
        if hit is None:
            return
        _bot_exit_alert_shown = True
    detail = (str(hit).strip() or "AI 服务异常")[:200]
    log_line(f"[ALERT] AI 故障导致机器人退出：{detail}")
    from utils.desktop_notifications import enqueue as enqueue_desktop_notification
    enqueue_desktop_notification(DATA_DIR, "BiliLearn AI 错误", f"AI 服务异常导致机器人停止：{detail}")
    tray = _system_tray
    if tray is not None:
        tray.notify("BiliLearn AI 错误", f"AI 服务异常导致机器人停止：{detail}")


def _notify_ai_error(source_line: str) -> None:
    """Rate-limited bottom-right system notification for non-fatal AI errors."""
    global _ai_error_notify_last, _ai_error_notify_last_msg
    detail = redact_sensitive_text(str(source_line or "").strip())[:160]
    if not detail:
        return
    now = time.time()
    with _ai_error_notify_lock:
        if now - _ai_error_notify_last < 60 and detail == _ai_error_notify_last_msg:
            return
        _ai_error_notify_last = now
        _ai_error_notify_last_msg = detail
    try:
        from utils.desktop_notifications import enqueue as enqueue_desktop_notification
        enqueue_desktop_notification(DATA_DIR, "BiliLearn AI 错误", detail)
    except Exception:
        pass
    tray = _system_tray
    if tray is not None:
        try:
            tray.notify("BiliLearn AI 错误", detail)
        except Exception:
            pass
...
h+='<div class="fg"><label>主动聊天</label><select id="cqActiveChat"><option value="1"'+(c.active_chat&&c.active_chat.enabled?' selected':'')+'>启用</option><option value="0"'+(!c.active_chat||!c.active_chat.enabled?' selected':'')+'>关闭</option></select></div>';
h+='<div class="fg"><label>主动聊天对象</label><select id="cqActiveTargetMode"><option value="any"'+(c.active_chat&&c.active_chat.target_mode==='owner'?'':' selected')+'>最近往来的用户</option><option value="owner"'+(c.active_chat&&c.active_chat.target_mode==='owner'?' selected':'')+'>只找主人（owner_bili_uid）</option></select></div>';
h+='<div class="fg"><label>触发概率（0-1，如 0.06=6%）</label><input id="cqActiveProb" type="number" step="0.01" min="0" max="1" value="'+((c.active_chat&&c.active_chat.prob_initiate!=null)?c.active_chat.prob_initiate:0.06)+'" style="width:110px"></div>';
h+='<div class="fg"><label>冷却分钟数</label><input id="cqActiveCooldown" type="number" min="1" value="'+((c.active_chat&&c.active_chat.cooldown_minutes!=null)?c.active_chat.cooldown_minutes:45)+'" style="width:110px"></div>';
h+='<div class="fg"><label>每轮上限次数</label><input id="cqActiveMax" type="number" min="1" value="'+((c.active_chat&&c.active_chat.max_initiate_per_session!=null)?c.active_chat.max_initiate_per_session:3)+'" style="width:110px"></div>';
h+='<div class="fg"><label>免打扰时段</label><input id="cqQuietStart" type="number" min="0" max="23" value="'+((c.active_chat&&c.active_chat.quiet_start_hour!=null)?c.active_chat.quiet_start_hour:22)+'" style="width:70px"> - <input id="cqQuietEnd" type="number" min="0" max="23" value="'+((c.active_chat&&c.active_chat.quiet_end_hour!=null)?c.active_chat.quiet_end_hour:8)+'" style="width:70px"> 时 <label class="toggle-sw" style="display:inline-flex;vertical-align:middle"><input id="cqQuietEn" type="checkbox"'+(c.active_chat&&c.active_chat.quiet_hours_enabled===false?'':' checked')+'><span class="toggle-track"></span></label>启用免打扰</div>';
h+='<div class="fg"><label>允许聊天时段（<label class="toggle-sw" style="display:inline-flex;vertical-align:middle"><input id="cqActiveHoursEn" type="checkbox"'+(c.active_chat&&c.active_chat.active_hours_enabled?' checked':'')+'><span class="toggle-track"></span></label>启用）</label><input id="cqActiveStart" type="number" min="0" max="23" value="'+((c.active_chat&&c.active_chat.active_start_hour!=null)?c.active_chat.active_start_hour:9)+'" style="width:70px"> - <input id="cqActiveEnd" type="number" min="0" max="23" value="'+((c.active_chat&&c.active_chat.active_end_hour!=null)?c.active_chat.active_end_hour:23)+'" style="width:70px"> 时</div>';
h+='<div class="fg"><label>自定义开场提示词（可选）</label><textarea id="cqActivePrompt" placeholder="例如：以猫娘口吻，先问候近况再找话题" style="width:100%;min-height:48px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:6px;color:var(--text);font-size:12px">'+esc(((c.active_chat||{}).custom_prompt||''))+'</textarea></div>';
h+='<div class="fg"><label>主动聊天白名单 UID（target_mode=any 时生效）</label><textarea id="cqActiveChatWhitelist" placeholder="留空则不限制；填写后只主动私信这些 UID（逗号或换行分隔）" style="width:100%;min-height:54px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:12px">'+(((c.active_chat||{}).whitelist_uids||[]).join('\n'))+'</textarea></div>';
h+='</fieldset>';
// Agent
...
c.active_chat.target_mode=document.getElementById('cqActiveTargetMode').value;
c.active_chat.prob_initiate=parseFloat(document.getElementById('cqActiveProb').value)||0.06;
c.active_chat.cooldown_minutes=parseInt(document.getElementById('cqActiveCooldown').value)||45;
c.active_chat.max_initiate_per_session=parseInt(document.getElementById('cqActiveMax').value)||3;
c.active_chat.quiet_hours_enabled=document.getElementById('cqQuietEn').checked;
c.active_chat.quiet_start_hour=parseInt(document.getElementById('cqQuietStart').value)||22;
c.active_chat.quiet_end_hour=parseInt(document.getElementById('cqQuietEnd').value)||8;
c.active_chat.active_hours_enabled=document.getElementById('cqActiveHoursEn').checked;
c.active_chat.active_start_hour=parseInt(document.getElementById('cqActiveStart').value)||9;
c.active_chat.active_end_hour=parseInt(document.getElementById('cqActiveEnd').value)||23;
c.active_chat.custom_prompt=(document.getElementById('cqActivePrompt').value||'').trim();
// Agent
c.agent=c.agent||{};
...
# ── 信息 ──
_SKIPPED_VERSION_FILE = DATA_DIR / "skipped_version.json"

def _read_skipped_version() -> str:
    """返回被用户跳过的更新版本号（空=未跳过）。"""
    try:
        d = json.loads(_SKIPPED_VERSION_FILE.read_text(encoding="utf-8", errors="replace")) if _SKIPPED_VERSION_FILE.exists() else {}
        return str(d.get("version") or "").strip()
    except Exception:
        return ""

def _write_skipped_version(version: str) -> None:
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        _SKIPPED_VERSION_FILE.write_text(
            json.dumps({"version": version or "", "at": __import__("time").strftime("%Y-%m-%d %H:%M:%S")}),
            encoding="utf-8")
    except Exception:
        pass
...
def api_check_update():
    """检查更新：请求 gengxin.bxya.app/v{当前版本}，智能判断是否未发布。"""
    import urllib.request as _ur, json as _json, re as _re3
    ...
    ver = current if current.startswith('v') else 'v' + current
    url = f"https://gengxin.bxya.app/{ver}"
    try:
        req = _ur.Request(url, headers={"User-Agent": "BiliLearn/"+ver, "Accept": "application/json"})
        with _ur.urlopen(req, timeout=12) as resp:
            data = _json.loads(resp.read().decode("utf-8", "replace"))
    except Exception as e:
        return jsonify(ok=True, current_version=ver, update_available=False,
                       error="检查更新失败: " + str(e), message="网络异常，无法连接更新服务器")
    latest = str(data.get("latest_version", "") or "")
    # 版本比较：服务器说 latest，若 latest 数字版本 >= 当前 → 有更新；否则（当前是未发布/开发版）→ 提示已是最新
    def vnum(s):
        m = _re3.findall(r"\d+", s)
        return tuple(int(x) for x in m[:3]) + (0,)*max(0,3-len(m))
    have_update = bool(latest) and vnum(latest) > vnum(current)
    skipped = _read_skipped_version()
    # 若该版本被用户跳过，不再提示更新（但仍回传状态，便于前端显示）
    if have_update and skipped and vnum(skipped) >= vnum(latest):
        have_update = False
    return jsonify(
        ok=True,
        current_version=ver,
        latest_version=latest,
        update_available=have_update,
        skipped_version=skipped,
        release_name=data.get("release_name", ""),
        release_body=data.get("release_body", ""),
        release_url=data.get("release_url", ""),
        server_unknown_current=bool(latest) and vnum(latest) < vnum(current),
        message=("发现新版本 " + latest if have_update else "当前已是最新版本"),
    )


@app.route('/api/project-intro')
def api_project_intro():
    """返回项目介绍页 HTML（当前版本）。"""
    import re as _re2
    project_intro = Path(BASE_DIR) / "project_intro.html"
    if not project_intro.exists():
        return jsonify(ok=False, message="项目介绍页不存在"), 404
...
def api_info():
    ...
    recorded_cost_total = float((costs or {}).get('total', 0) or 0)
    try:
        from services.quota_alerts import maybe_alert_recorded_spend_async
        maybe_alert_recorded_spend_async(recorded_cost_total)
    except Exception:
        pass
    ...
    return jsonify(dict(
        ...
        bili_uid=str((read_json(COOKIE_FILE, {}) or {}).get('DedeUserID') or '')[:32],
        ...
        cost_total=recorded_cost_total,
        files=files,
...
def api_watch_later():
    """Bilibili watch-later list. GET is read-only; writes honour the toggle."""
    from core.config import load_config
    from services import social_center
    try:
        if request.method == 'GET':
            return jsonify(ok=True, items=social_center.list_watch_later())
        if not bool(load_config().get('interaction', {}).get('enable_watch_later', True)):
            return jsonify(ok=False, message='稍后再看功能已关闭'), 403
        body = request.get_json(silent=True) or {}
        ...
        if request.method == 'POST':
            ...
            return jsonify(ok=True, result=social_center.add_watch_later(bvid), message='已加入稍后再看')
        if not bvid:
            return jsonify(ok=False, message='缺少 BV 号'), 400
        return jsonify(ok=True, result=social_center.remove_watch_later(bvid), message='已从稍后再看移除')
    except Exception as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 400


@app.route('/api/watch-later/clear', methods=['POST'])
def api_watch_later_clear():
    from core.config import load_config
    from services import social_center
    if not bool(load_config().get('interaction', {}).get('enable_watch_later', True)):
        return jsonify(ok=False, message='稍后再看功能已关闭'), 403
    try:
        return jsonify(ok=True, result=social_center.clear_watch_later(), message='稍后再看列表已清空')
    except Exception as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 400
...
def api_reminders():
    ...
    if request.method == 'GET':
        return jsonify(ok=True, items=reminders.list_reminders())
    body = request.get_json(silent=True) or {}
    try:
        if request.method == 'POST':
            item = reminders.create_reminder(str(body.get('content') or ''), str(body.get('due_at') or ''), owner_uid='manual')
            return jsonify(ok=True, item=item, message='待办已保存')
        deleted = reminders.remove_reminder(str(body.get('id') or ''))
        return jsonify(ok=deleted, message='待办已删除' if deleted else '没有找到待办')
...
@app.route('/api/dynamics/settings', methods=['GET', 'POST'])
def api_dynamics_settings():
    """自动发布设置：时段/每日限额/内容来源/自定义提示词。"""
    cfg = read_json(CONFIG_FILE, {})
    da = cfg.setdefault("dynamic_auto", {})
    if request.method == 'GET':
        return jsonify(ok=True, settings={
            "enabled": bool(da.get("enabled", False)),
            "start_hour": int(da.get("start_hour", 18)),
            "end_hour": int(da.get("end_hour", 22)),
            "daily_max": int(da.get("daily_max", 3)),
            "daily_min": int(da.get("daily_min", 1)),
            "sources": da.get("sources", ["private_message", "comment", "watched_video", "recent_events"]),
            "custom_prompt": str(da.get("custom_prompt", "")),
        })
    body = request.get_json(silent=True) or {}
    if "enabled" in body: da["enabled"] = bool(body["enabled"])
    if "start_hour" in body: da["start_hour"] = max(0, min(23, int(body["start_hour"])))
    if "end_hour" in body: da["end_hour"] = max(0, min(23, int(body["end_hour"])))
    if "daily_max" in body: da["daily_max"] = max(1, min(20, int(body["daily_max"])))
    if "daily_min" in body: da["daily_min"] = max(0, min(20, int(body["daily_min"])))
    if "sources" in body:
        allowed = {"private_message", "comment", "watched_video", "recent_events"}
        da["sources"] = [s for s in body["sources"] if s in allowed]
    if "custom_prompt" in body: da["custom_prompt"] = str(body["custom_prompt"])[:2000]
    if not write_json(CONFIG_FILE, cfg):
        return jsonify(ok=False, message="设置保存失败"), 500
    return jsonify(ok=True, message="自动发布设置已保存")


@app.route('/api/dynamics/test-publish', methods=['POST'])
def api_dynamics_test_publish():
    """测试发一条动态（真实发布，受 AI 行为审核管控）。"""
    body = request.get_json(silent=True) or {}
    text = str(body.get("text") or "").strip()
    if not text:
        return jsonify(ok=False, message="请输入要发布的动态内容"), 400
    try:
        from core.config import load_config
        from services.like_review import ActionReviewInbox, requires_review
        config = load_config()
        if requires_review(config, 'dynamic_publish'):
            proposed = ActionReviewInbox(DATA_DIR).propose(
                'dynamic_publish', '发布测试动态', text[:200],
                {'test_text': text}, dedupe_key=f'dynamic_test_publish:{text[:80]}')
            return jsonify(ok=True, queued=True, item=proposed, message='已进入 AI 行为审核，审核通过后才会发布')
        from services import social_center
        result = social_center.run_async(
            social_center.publish_test_dynamic(text))
        if result.get("executed"):
            return jsonify(ok=True, message="测试动态已发布成功", data=result)
        return jsonify(ok=False, message=str(result.get("reason") or "发布失败"), data=result)
    except Exception as exc:
        return jsonify(ok=False, message="发布失败: " + str(exc)), 500

@app.route('/api/dynamic-publish-log')
def api_dynamic_publish_log():
    """动态发布记录（含草稿发布与测试发布）。"""
    try:
        from services import social_center
        path = Path(DATA_DIR) / "dynamic_publish_log.json"
        rows = read_json(path, [])
        if not isinstance(rows, list):
            rows = []
        rows = rows[::-1]  # 新的在前
        # 补充 dyn_id 展示：result 里带 dyn_id/dyn_rid
        out = []
        for row in rows[:200]:
            ...
            out.append({
                ...
                "dyn_id": str(result.get("dyn_id") or ""),
                "dyn_rid": str(result.get("dyn_rid") or ""),
...
@app.route('/api/dynamic-drafts', methods=['GET', 'POST', 'DELETE'])
def api_dynamic_drafts():
    from core.config import load_config
    from services import social_center
    if request.method == 'GET':
        return jsonify(ok=True, items=social_center.list_dynamic_drafts())
    if not bool(load_config().get('interaction', {}).get('enable_dynamic_draft', True)):
        return jsonify(ok=False, message='动态草稿功能已关闭'), 403
    body = request.get_json(silent=True) or {}
    try:
        if request.method == 'POST':
            return jsonify(ok=True, item=social_center.save_dynamic_draft(body), message='动态草稿已保存')
        deleted = social_center.delete_dynamic_draft(str(body.get('id') or ''))
        return jsonify(ok=deleted, message='草稿已删除' if deleted else '没有找到草稿')
    except ValueError as exc:
        return jsonify(ok=False, message=str(exc)), 400


@app.route('/api/dynamic-drafts/<draft_id>/publish', methods=['POST'])
def api_dynamic_draft_publish(draft_id):
    from core.config import load_config
    from services.like_review import ActionReviewInbox, requires_review
    from services import social_center
    config = load_config()
    if not bool(config.get('interaction', {}).get('enable_dynamic_publish', False)):
        return jsonify(ok=False, message='动态发布功能未开启'), 403
    draft = next((row for row in social_center.list_dynamic_drafts() if str(row.get('id')) == draft_id), None)
    if not draft:
        return jsonify(ok=False, message='没有找到草稿'), 404
    if requires_review(config, 'dynamic_publish'):
        proposed = ActionReviewInbox(DATA_DIR).propose('dynamic_publish', '发布动态草稿', draft.get('text', '')[:200], {'draft_id': draft_id}, dedupe_key='dynamic:' + draft_id)
        return jsonify(ok=True, queued=True, item=proposed, message='已进入 AI 行为审核，审核通过后才会发布')
    try:
        return jsonify(ok=True, result=social_center.publish_dynamic_draft(draft_id), message='动态已发布')
    except Exception as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 400
...
def api_config():
    if request.method=='GET':
        from core.config import load_config
        value = load_config()
        # Never expose the encrypted SMTP password in the general config editor.
        quota = value.get("quota_alert")
        if isinstance(quota, dict) and quota.get("smtp_password_encrypted"):
            quota["smtp_password_encrypted"] = "[已隐藏]"
        # Runtime safety rules are deliberately backend-only. Keep placeholders
        # as strings so a later general-config save cannot erase stored rules.
        reply_safety = value.get("reply_safety")
        if isinstance(reply_safety, dict) and "blocked_keywords" in reply_safety:
            reply_safety["blocked_keywords"] = "[已隐藏]"
        prompt_injection = value.get("prompt_injection")
        if isinstance(prompt_injection, dict) and "custom_terms" in prompt_injection:
            prompt_injection["custom_terms"] = "[已隐藏]"
...
def api_quota_alert_settings():
    from services.quota_alerts import save_settings, status
    try:
        if request.method == 'GET':
            return jsonify(status())
        return jsonify(ok=True, settings=save_settings(request.get_json(silent=True) or {}), message='额度提醒设置已保存')
    except ValueError as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 400
    except Exception as exc:
...
@app.route('/api/quota-alert/status')
def api_quota_alert_status():
    from services.quota_alerts import status
    return jsonify(status())


@app.route('/api/quota-alert/test', methods=['POST'])
def api_quota_alert_test():
    from services.quota_alerts import send_test_email
    try:
        send_test_email()
        ...
        return jsonify(ok=False, message='测试邮件发送失败: ' + redact_sensitive_text(str(exc))), 502
...
def api_kb_path():
    ...
    config = read_json(CONFIG_FILE, {})
    if request.method == 'GET':
        current = str(resolve_knowledge_base_dir(config))
        custom = config.get("knowledge_base_dir") or config.get("knowledge", {}).get("base_dir") or ""
        return jsonify(ok=True, current_path=current, custom_path=custom)
    body = request.get_json(silent=True) or {}
    path = (body.get("path") or "").strip()
    if path:
        config["knowledge_base_dir"] = path
    else:
        config.pop("knowledge_base_dir", None)
    save_config(config)
    return jsonify(ok=True, message="知识库路径已保存，重启面板后生效" if path else "已恢复默认路径")


def _storage_groups_payload(root: Path) -> dict:
    groups = {
        "data": ("运行数据", root / "Data"),
        "knowledge": ("知识库", root / "KnowledgeBase"),
        "highlights": ("视频高光与图片", root / "highlights"),
        "html_exports": ("网页导出", root / "html_exports"),
        "mindmaps": ("思维导图", root / "MindMaps"),
        "documents": ("文档导出", root / "Word"),
        "qr_codes": ("登录二维码", root / "qr_codes"),
    }
    result = {}
    for key, (label, path) in groups.items():
        try:
            ...
            files, size = 0, 0
        result[key] = {"label": label, "path": str(path), "exists": path.exists(), "files": files, "bytes": size}
    return result
...
def api_storage():
    ...
    if request.method == 'GET':
        return jsonify(ok=True, current_path=str(USER_DATA_DIR), configured_path=str(USER_DATA_DIR),
                       restart_required=False, groups=_storage_groups_payload(Path(USER_DATA_DIR)))
    body = request.get_json(silent=True) or {}
    raw = str(body.get("path") or "").strip()
    target = default_root.resolve() if not raw else Path(raw).expanduser().resolve()
    if len(str(target)) < 4 or target == target.parent:
        return jsonify(ok=False, message="存储目录过短或不能使用磁盘根目录"), 400
    try:
        target.mkdir(parents=True, exist_ok=True)
        if body.get("migrate") is True and Path(USER_DATA_DIR).resolve() != target:
            import shutil
            shutil.copytree(Path(USER_DATA_DIR), target, dirs_exist_ok=True)
        pointer.parent.mkdir(parents=True, exist_ok=True)
        pointer.write_text(json.dumps({"path": str(target), "updated_at": datetime.now().isoformat(timespec="seconds")}, ensure_ascii=False, indent=2), encoding="utf-8")
        return jsonify(ok=True, message="数据目录已保存，重启面板后生效", current_path=str(USER_DATA_DIR), configured_path=str(target), restart_required=True,
                       groups=_storage_groups_payload(target))
    except OSError as exc:
        return jsonify(ok=False, message=f"保存数据目录失败：{exc}"), 400
...
def api_storage_open():
    ...
    target = Path(groups[key]['path']).resolve()
    root = Path(USER_DATA_DIR).resolve()
    if root not in target.parents and target != root:
        return jsonify(ok=False, message='存储路径校验失败'), 400
    try:
        target.mkdir(parents=True, exist_ok=True)
        if os.name == 'nt':
...
def api_storage_pick():
    """Open the native file picker so paths never need to be typed into the panel."""
    body = request.get_json(silent=True) or {}
    mode = str(body.get('mode') or 'directory').strip().lower()
    initial = str(body.get('initial') or USER_DATA_DIR).strip()
    try:
        ...
        if mode == 'file':
            selected = filedialog.askopenfilename(
                parent=root,
                title=str(body.get('title') or '选择本地文件'),
                initialdir=initial if Path(initial).is_dir() else str(Path(initial).parent),
                filetypes=[
            ...
            selected = filedialog.askdirectory(
                ...
                initialdir=initial if Path(initial).is_dir() else str(USER_DATA_DIR),
                mustexist=False,
            )
        root.destroy()
        if not selected:
            return jsonify(ok=False, cancelled=True, message='已取消选择')
...
def api_logs_export():
    """Download the currently selected, redacted log stream as TXT or JSON."""
    source = (request.args.get('source') or 'all').strip().lower()
    fmt = (request.args.get('format') or 'txt').strip().lower()
    if source not in {'all', 'bot', 'monitor', 'reviews'} or fmt not in {'txt', 'json'}:
        return jsonify(ok=False, message='日志导出参数不正确'), 400
    with app.test_request_context(f'/api/logs?source={source}&limit=5000'):
        payload = api_logs().get_json()
    lines = payload.get('lines', []) if isinstance(payload, dict) else []
    stamp = datetime.now().strftime('%Y%m%d-%H%M%S')
    if fmt == 'json':
        content = json.dumps({'exported_at': datetime.now().isoformat(), 'source': source, 'lines': lines}, ensure_ascii=False, indent=2)
        mime, suffix = 'application/json; charset=utf-8', 'json'
    else:
        content = '\n'.join(f"[{item.get('source', '')}] {item.get('text', '')}" for item in lines)
        mime, suffix = 'text/plain; charset=utf-8', 'txt'
    response = app.response_class(content, mimetype=mime)
    response.headers['Content-Disposition'] = f'attachment; filename="bililearn-{source}-logs-{stamp}.{suffix}"'
    return response
...
def api_memory():
    ...
    return jsonify(dict(
        ...
        permanent_memories=MemoryBank().list_permanent(limit=500),
        relationships=MemoryBank().contacts(limit=500),
    ))


@app.route('/api/permanent-memory', methods=['GET', 'POST', 'PUT', 'DELETE'])
def api_permanent_memory():
    """CRUD for explicit, user-managed long-term memories."""
    bank = MemoryBank()
    try:
        if request.method == 'GET':
            query = str(request.args.get('q') or '').strip()
            rows = bank.search(query, limit=200) if query else bank.list_permanent(limit=500)
            items = [row for row in rows if row.get('permanent')]
            # 合并 AI 侧 PrivateContextDB 的长期记忆（面板与 AI 两套系统互通）
            try:
                from persona.managers import PrivateContextDB
                ctx = PrivateContextDB()
                ctx_mem = ctx._memories if isinstance(getattr(ctx, "_memories", None), dict) else {}
                for uid, entries in ctx_mem.items():
                    if not isinstance(entries, list):
                        continue
                    for entry in entries:
                        ...
                        if not content:
                            continue
                        items.append({
                            "id": f"ctx:{uid}:{entry.get('time') or ''}",
                            "content": content,
                            "summary": content[:160],
                            "user_id": str(uid),
                            "permanent": True,
                            "tags": list(entry.get("tags") or []) + ["AI记忆"],
                            "created_at": str(entry.get("time") or ""),
                            "updated_at": str(entry.get("time") or ""),
                            "source": "ai_context",
                        })
            except Exception:
                pass
        ...
        if request.method == 'POST':
            ...
            if not content:
                return jsonify(ok=False, message='记忆内容不能为空'), 400
            item = bank.add(content, user_id=str(body.get('user_id') or 'local'), thread_id='permanent', permanent=True, tags=body.get('tags') or [])
            return jsonify(ok=True, item=item), 201
        memory_id = str(body.get('id') or request.args.get('id') or '').strip()
        if not memory_id:
            return jsonify(ok=False, message='缺少记忆 id'), 400
        if request.method == 'DELETE':
            return jsonify(ok=True, deleted=bank.delete(memory_id))
        item = bank.update(memory_id, content=body.get('content'), summary=body.get('summary'), tags=body.get('tags'), permanent=True)
        if not item:
            return jsonify(ok=False, message='记忆不存在'), 404
        return jsonify(ok=True, item=item)
    except (TypeError, ValueError) as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 400


@app.route('/api/relationships', methods=['GET', 'POST', 'DELETE'])
def api_relationships():
    bank = MemoryBank()
    try:
        if request.method == 'GET':
            query = str(request.args.get('q') or '').strip()
            rows = bank.contacts(query=query, limit=500)
            # 合并 AI 侧 PrivateContextDB 的 B 友画像（面板与 AI 两套系统互通）
            try:
                from persona.managers import PrivateContextDB
                ctx = PrivateContextDB()
                ctx_prof = ctx._profiles if isinstance(getattr(ctx, "_profiles", None), dict) else {}
                existing_uids = {str(row.get("uid")) for row in rows if isinstance(row, dict)}
                for uid, prof in ctx_prof.items():
                    if not isinstance(prof, dict) or str(uid) in existing_uids:
                        continue
                    if str(uid) in ("_memories", "_profiles"):
                        continue
                    rows.append({
                        "uid": str(uid),
                        "name": str(prof.get("display_name") or prof.get("user_uid") or f"用户{uid}"),
                        "avatar": "",
                        "interest_types": [],
                        "chat_style": f"互动 {prof.get('interactions', 0)} 次 · 好感 {prof.get('affinity', 0)} · 最近渠道 {prof.get('last_channel', '?')}",
                        "notes": f"AI自动记录 · 首次互动 {str(prof.get('first_seen') or '')[:19]}",
                        "source": "ai_context",
                    })
        ...
        body = request.get_json(silent=True) or {}
        uid = str(body.get('uid') or request.args.get('uid') or '').strip()
        if request.method == 'DELETE':
            if not uid:
                return jsonify(ok=False, message='缺少 UID'), 400
            return jsonify(ok=True, deleted=bank.delete_contact(uid))
        if not uid:
            return jsonify(ok=False, message='缺少 UID'), 400
        item = bank.upsert_contact(uid, **body)
        return jsonify(ok=True, item=item)
    except (TypeError, ValueError) as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 400
...
def api_vision_settings():
    """Expose one multimodal master switch while preserving legacy keys."""
    from core.config import load_config, save_config
    config = load_config()
    vision = config.setdefault('vision', {})
    if request.method == 'POST':
        body = request.get_json(silent=True) or {}
        enabled = bool(body.get('multimodal_enabled'))
        vision['multimodal_enabled'] = enabled
        for key in ('cover_enabled', 'frames_enabled', 'comment_images_enabled'):
            vision[key] = enabled
        save_config(config)
    enabled = bool(vision.get('multimodal_enabled', False))
    return jsonify(ok=True, multimodal_enabled=enabled)
...
def _activity_from_runtime_logs(lines: list[str]) -> dict:
    """Best-effort status: prefer real-time state file, fall back to log parsing."""
    try:
        ...
        if state_file.exists():
            ...
            if ts:
                ...
                if datetime.now() - state_time < timedelta(minutes=5):
                    return {"label": state_data.get("state", "运行中"), "detail": state_data.get("detail", ""), "bvid": state_data.get("bvid", ""), "title": state_data.get("title", "")}
    except Exception:
        pass
    text = "\n".join(lines[-12:])
    checks = (
...
def _favorite_payload() -> dict:
    ...
    for folder in library["folders"]:
        ...
        for item in library["items"]:
            ...
            card["ai_category"] = str(item.get("ai_category") or "")
            card["ai_category_at"] = str(item.get("ai_category_at") or "")
            items.append(card)
...
@app.route('/api/favorites/items', methods=['GET', 'POST', 'DELETE'])
def api_favorite_items():
    if request.method == 'GET':
        # 供 v2w 等来源选择：返回扁平收藏项列表（bvid/title/up_name）
        ...
        for folder in payload.get("folders", []):
            for it in folder.get("items", []):
                ...
                items.append({
                    "bvid": it["bvid"],
                    "title": str(it.get("title") or it["bvid"]),
                    "up_name": str(it.get("up") or "未知 UP"),
                    "folder_id": str(it.get("folder_id") or folder.get("id") or ""),
                })
    ...
    if not any(str(item.get("folder_id")) == folder_id and _safe_watch_bvid(item.get("bvid")) == bvid for item in library["items"]):
        ...
        _write_favorite_library(library)
        # 后台异步 AI 归类（不阻塞收藏响应）
        def _bg_classify(bvid_=bvid):
            try:
                from services.local_favorites import read_library as _rl, write_library as _wl, classify_favorite_item
                lib = _rl()
                for it in lib.get("items") or []:
                    if str(it.get("bvid") or "").strip() == bvid_ and not str(it.get("ai_category") or "").strip():
                        loop2 = asyncio.new_event_loop()
                        try:
                            it["ai_category"] = loop2.run_until_complete(classify_favorite_item(it))
                        finally:
                            loop2.close()
                        it["ai_category_at"] = datetime.now().isoformat(timespec="seconds")
                        break
                _wl(lib)
            except Exception:
                pass
...
@app.route('/api/favorites/classify', methods=['POST'])
def api_favorites_classify():
    """Batch AI-classify every favorite video that lacks an ai_category."""
    try:
        from services.local_favorites import classify_all_favorites
        loop = asyncio.new_event_loop()
        try:
            result = loop.run_until_complete(classify_all_favorites())
        finally:
            loop.close()
...
def api_export():
    try:
        ...
        body = request.get_json(silent=True) or {}
        export_root = BACKUP_DIR_EXPORT
        requested_root = str(body.get('destination_dir') or '').strip()
        if requested_root:
            export_root = Path(requested_root).expanduser().resolve()
            if len(str(export_root)) < 4 or export_root == export_root.parent:
                return jsonify(ok=False, message='导出目录不能是磁盘根目录'), 400
        ...
        if selected != ['settings']:
            import zipfile
            out = export_root / f"bilibili_learning_bot_backup_{ts}.zip"
        ...
        out = export_root / f"bilibili_learning_bot_export{suffix}_{ts}.json"
        if sanitize:
...
def _execute_review_action(item: dict) -> dict:
    ...
    elif action_type == 'dynamic_publish':
        from services.social_center import publish_dynamic_draft
        if payload.get('test_text'):
            from services import social_center
            result = social_center.publish_test_dynamic(str(payload.get('test_text') or ''))
        else:
...
@app.route('/api/mindmaps/export', methods=['POST'])
def api_mindmaps_export():
    """Copy one generated mind map to a directory explicitly chosen by the user."""
    body = request.get_json(silent=True) or {}
    requested = Path(str(body.get('path') or '')).expanduser().resolve()
    destination = Path(str(body.get('destination_dir') or '')).expanduser().resolve()
    from core.user_data import MINDMAPS_DIR
    source_root = Path(MINDMAPS_DIR).resolve()
    if not requested.is_file() or source_root not in requested.parents or not requested.name.endswith('.mindmap.html'):
        return jsonify(ok=False, message='思维导图不存在或路径不允许'), 404
    if len(str(destination)) < 4 or destination == destination.parent:
        return jsonify(ok=False, message='请选择具体的导出文件夹'), 400
    try:
        import shutil
        destination.mkdir(parents=True, exist_ok=True)
        target = destination / requested.name
        shutil.copy2(requested, target)
        return jsonify(ok=True, message=f'已导出到 {target}', path=str(target))
    except OSError as exc:
        return jsonify(ok=False, message=f'导出思维导图失败: {redact_sensitive_text(str(exc))}'), 500
...
def api_action_toggle_flag():
    ...
    try:
        ...
        allowed = {
            ...
            'multimodal_enabled': ('vision', 'multimodal_enabled', 'VISION_MULTIMODAL_ENABLED'),
            'quick_mode': ('speed', 'no_human_delay', 'NO_HUMAN_DELAY'),
            'quiet_mode': ('system', 'quiet_mode', 'QUIET_MODE'),
            'asr_enabled': ('asr', 'enabled', 'ASR_ENABLED'),
            'cookie_preheat': ('speed', 'cookie_preheat', 'COOKIE_PREHEAT'),
        }
        ...
        # cookie_preheat 默认开启，其余默认关闭
        default_on = key == 'cookie_preheat'
        new_val = not bool(sec.get(field, default_on))
        sec[field] = new_val
        if key == 'multimodal_enabled':
            for visual_key in ('cover_enabled', 'frames_enabled', 'comment_images_enabled'):
                sec[visual_key] = new_val
        write_json(CONFIG_FILE, config)
        try:
            ...
            pass
        label = {'cover_enabled': '封面分析', 'multimodal_enabled': '多模态识图', 'quick_mode': '快速模式',
                 'quiet_mode': '安静模式', 'asr_enabled': 'ASR语音识别', 'cookie_preheat': 'Cookie预热'}[key]
...
def api_action_flags():
    ...
    try:
        ...
        return jsonify(dict(
            ...
            multimodal_enabled=bool(config.get('vision', {}).get('multimodal_enabled', False)),
            ...
            cookie_preheat=bool(config.get('speed', {}).get('cookie_preheat', True)),
        ))
    except Exception as e:
        return jsonify(dict(ok=False, message=str(e))), 400
...
class TaskCancelled(Exception):
    """Raised inside a task worker when the user asked to stop it."""


def _task_cancelled(task_id) -> bool:
    with _TASKS_LOCK:
        task = TASKS.get(task_id)
        return bool(task and task.get('cancel_requested'))


def _cancel_task(task_id) -> bool:
    """Mark a running task as cancelled; its worker checks and aborts soon."""
    with _TASKS_LOCK:
        task = TASKS.get(task_id)
        if not task:
            return False
...
def _run_coro(coro, task_id=None):
    loop = _asyncio.new_event_loop()
    try:
        _asyncio.set_event_loop(loop)
        if task_id:
            return loop.run_until_complete(_cancellable(coro, task_id, loop))
        return loop.run_until_complete(coro)
    finally:
        loop.close()


async def _cancellable(coro, task_id, loop):
    """Run coro while watching the cancel flag; abort between awaits."""
    main = _asyncio.ensure_future(coro)
    while True:
        if _task_cancelled(task_id):
            ...
            try:
                ...
                raise TaskCancelled('用户已停止该任务')
        done, _pending = await _asyncio.wait({main}, timeout=0.4)
        if main in done:
            return main.result()
...
def api_action_video2web():
    ...
    try:
        ...
        nav_buttons = bool(body.get('nav_buttons', True))
        ...
        def _work(tid):
            ...
            res = _run_coro(generate_ppt_from_bvid(
                ...
                enhanced_animations=enhanced_animations, watermark=watermark, slide_count=slide_count,
...
@app.route('/api/action/task/cancel', methods=['POST'])
def api_action_task_cancel():
    """Request graceful cancellation of a running background task."""
    body = request.get_json(silent=True) or {}
    tid = str(body.get('id') or '')
    if not tid:
        return jsonify(dict(ok=False, message='缺少任务 ID')), 400
    cancelled = _cancel_task(tid)
    return jsonify(dict(ok=cancelled, message='已请求停止' if cancelled else '任务不存在或已完成'))
...
def api_action_task():
    ...
    with _TASKS_LOCK:
        ...
        if snapshot.get('cancel_requested') and snapshot.get('status') not in ('done', 'error'):
...
def _agent_workspace_config() -> dict:
    ...
    return {
        ...
        'auto_tools': agent.get('auto_tools', agent.get('auto_enabled', True)),
        'max_steps': max(1, min(12, int(agent.get('max_steps_per_plan', 5) or 5))),
        'custom_prompt': str(agent.get('custom_prompt', '') or '')[:4000],
        'mcp_servers': agent.get('mcp_servers', []) if isinstance(agent.get('mcp_servers'), list) else [],
    }


def _agent_skills_file() -> Path:
    return Path(DATA_DIR) / 'agent_skills.json'


def _agent_skills() -> list:
    rows = read_json(_agent_skills_file(), [])
...
def api_agent_workspace():
    if request.method == 'GET':
        return jsonify(ok=True, config=_agent_workspace_config(), skills=_agent_skills())
    try:
        body = request.get_json(silent=True) or {}
        config = read_json(CONFIG_FILE, {})
        agent = config.setdefault('agent', {})
        agent['enabled'] = body.get('enabled') is not False
        agent['auto_tools'] = body.get('auto_tools') is not False
        agent['max_steps_per_plan'] = max(1, min(12, int(body.get('max_steps', 5) or 5)))
        agent['custom_prompt'] = str(body.get('custom_prompt', '') or '').strip()[:4000]
        write_json(CONFIG_FILE, config)
        return jsonify(ok=True, message='Agent 配置已保存', config=_agent_workspace_config())
    except (TypeError, ValueError) as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 400


@app.route('/api/agent/skills', methods=['POST'])
def api_agent_skills_create():
    body = request.get_json(silent=True) or {}
    name = str(body.get('name', '') or '').strip()[:120]
    summary = str(body.get('summary', '') or '').strip()[:4000]
    if not name or not summary:
        return jsonify(ok=False, message='请填写 Skill 名称和用途'), 400
    rows = _agent_skills()
    item = {'id': f'skill-{int(time.time() * 1000)}', 'name': name, 'summary': summary,
            'source': str(body.get('source', 'custom') or 'custom')[:32],
            'bvid': str(body.get('bvid', '') or '')[:32], 'created_at': datetime.now().isoformat(timespec='seconds')}
    rows.insert(0, item)
    write_json(_agent_skills_file(), rows[:200])
    return jsonify(ok=True, message='Skill 已保存', skill=item)


@app.route('/api/agent/skills/<skill_id>', methods=['DELETE'])
def api_agent_skills_delete(skill_id):
    rows = _agent_skills()
    kept = [row for row in rows if str(row.get('id')) != skill_id]
    if len(kept) == len(rows):
        return jsonify(ok=False, message='Skill 不存在'), 404
    write_json(_agent_skills_file(), kept)
    return jsonify(ok=True, message='Skill 已删除')


@app.route('/api/agent/skills/summarize', methods=['POST'])
def api_agent_skills_summarize():
    body = request.get_json(silent=True) or {}
    bvid = _safe_watch_bvid(body.get('bvid'))
    source = str(body.get('source', 'video') or 'video')
    if not bvid:
        return jsonify(ok=False, message='请输入有效 BV 号'), 400
    metadata = _fetch_watch_history_metadata(bvid)
    if not metadata:
        return jsonify(ok=False, message='未能读取该视频的公开资料，暂未生成 Skill'), 502
    prompt = ('根据以下公开视频资料提炼一个可复用的 Agent Skill。不要声称已经观看或执行任何账号操作。'
              '用中文输出：Skill 名称、适用场景、执行步骤、边界条件。\n\n'
              f'BV: {bvid}\n分区: {metadata.get("category", "")}\n简介: {metadata.get("description", "")}')
    try:
        from services._services_ai import call_ai
        summary = _run_coro(call_ai([
            {'role': 'system', 'content': '你是谨慎的 Agent 工作流设计师，只基于给定资料写可复用 Skill。'},
            {'role': 'user', 'content': prompt},
        ], temperature=0.2, max_tokens=700, verbose=False)).strip()
    except Exception as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 502
    if not summary:
        return jsonify(ok=False, message='AI 没有返回可用的 Skill 总结'), 502
    rows = _agent_skills()
    item = {'id': f'skill-{int(time.time() * 1000)}', 'name': f'{source}：{bvid}', 'summary': summary[:4000],
            'source': source, 'bvid': bvid, 'created_at': datetime.now().isoformat(timespec='seconds')}
    rows.insert(0, item)
    write_json(_agent_skills_file(), rows[:200])
    return jsonify(ok=True, message='AI 已总结并保存 Skill', skill=item)


@app.route('/api/agent/mcp', methods=['POST'])
def api_agent_mcp_create():
    body = request.get_json(silent=True) or {}
    name = str(body.get('name', '') or '').strip()[:120]
    endpoint = str(body.get('endpoint', '') or '').strip()[:1000]
    parsed = urlsplit(endpoint)
    if not name or parsed.scheme not in {'http', 'https'} or not parsed.netloc:
        return jsonify(ok=False, message='请填写服务名称和有效 HTTP(S) 地址'), 400
    if parsed.username or parsed.password:
        return jsonify(ok=False, message='MCP 地址不能包含账号或密钥'), 400
    config = read_json(CONFIG_FILE, {})
    agent = config.setdefault('agent', {})
...
@app.route('/api/agent/mcp/<mcp_id>', methods=['DELETE'])
def api_agent_mcp_delete(mcp_id):
    config = read_json(CONFIG_FILE, {})
    agent = config.setdefault('agent', {})
    rows = agent.get('mcp_servers', []) if isinstance(agent.get('mcp_servers'), list) else []
    kept = [row for row in rows if str(row.get('id')) != mcp_id]
    if len(kept) == len(rows):
...
def api_kb_export():
    """导出知识库：打包 KnowledgeBase 下所有 .md 文件为 zip 下载。"""
    try:
        import zipfile, io
        from services.knowledge_tutor import scan_md_files
        kb_dir = active_knowledge_base_dir()
        files = scan_md_files(kb_dir)
        if not files:
            return jsonify(ok=False, message="知识库为空，没有可导出的文件"), 404
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as archive:
            ...
            for item in files:
                ...
                added += 1
        if added == 0:
            return jsonify(ok=False, message="知识库中没有可导出的文件"), 404
        buf.seek(0)
        from datetime import datetime as _dt
        stamp = _dt.now().strftime("%Y%m%d_%H%M")
        return send_file(buf, mimetype='application/zip',
                         as_attachment=True,
                         download_name=f"BiliLearn_知识库导出_{stamp}.zip")
    except Exception as e:
        return jsonify(ok=False, message=f"导出失败: {e}"), 500


@app.route('/api/kb/export-file', methods=['POST'])
def api_kb_export_file():
    """Export one knowledge note as Markdown, TXT, JSON, or a readable PNG."""
    try:
        body = request.get_json(silent=True) or {}
        rel_path = str(body.get("rel_path") or "").strip().replace("\\", "/")
        fmt = str(body.get("format") or "md").strip().lower()
        if fmt not in {"md", "txt", "json", "png"}:
            return jsonify(ok=False, message="仅支持 md、txt、json、png"), 400
        from services.knowledge_tutor import read_md_file, scan_md_files
        indexed = {str(item.get("rel_path") or "").replace("\\", "/"): item for item in scan_md_files(active_knowledge_base_dir())}
        item = indexed.get(rel_path)
        if not item or not item.get("file_path"):
            return jsonify(ok=False, message="知识文件不存在"), 404
        source = Path(item["file_path"]).resolve()
        content = read_md_file(source)
        ...
        if fmt == "png":
            ...
            candidates = [Path(font_raw)] if font_raw else []
            candidates += [Path(r"C:\Windows\Fonts\msyh.ttc"), Path(r"C:\Windows\Fonts\simhei.ttf"), Path(r"C:\Windows\Fonts\segoeui.ttf")]
            def load_font(size):
                for candidate in candidates:
                    try:
                        if candidate.exists() and candidate.suffix.lower() in {".ttf", ".ttc", ".otf"}:
                            return ImageFont.truetype(str(candidate), size)
                    except OSError:
                        continue
                return ImageFont.load_default()
            title_font, body_font, small_font = load_font(48), load_font(27), load_font(21)
            draw.text((120, 115), title, font=title_font, fill=(80, 42, 68, 255))
            meta = " · ".join(str(v) for v in (item.get("category_path") or "知识库", item.get("up_name") or "", item.get("bvid") or "") if v)
            draw.text((125, 185), meta, font=small_font, fill=(135, 95, 120, 255))
            max_chars = 42
            lines = []
            ...
            for line in lines[:25]:
                draw.text((125, y), line, font=body_font, fill=(54, 48, 56, 255))
                y += 29
            draw.text((125, height - 115), "BiliLearn · 本地知识导出", font=small_font, fill=(150, 115, 140, 230))
            output = io.BytesIO()
        ...
        if fmt == "json":
            payload = dict(item=item, content=content)
        ...
        output.seek(0)
        mimetype = {"md": "text/markdown", "txt": "text/plain", "json": "application/json"}[fmt]
        return send_file(output, mimetype=mimetype, as_attachment=True, download_name=f"{safe_name}.{fmt}")
    except Exception as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 500
...
def api_kb_list_files():
    ...
    try:
        ...
        for item in files:
            bvid = _safe_watch_bvid(item.get("bvid"))
            if not bvid:
                # Older notes did not always write front matter. Recover BV IDs
                # from their visible filename/title so those cards can use the
                # same public-cover cache as normal watch history.
                source_text = " ".join(str(item.get(key) or "") for key in ("title", "rel_path", "file_path"))
                match = re.search(r"\b(BV[0-9A-Za-z]{8,20})\b", source_text, re.I)
                bvid = _safe_watch_bvid(match.group(1)) if match else ""
                if bvid:
                    item["bvid"] = bvid
...
def api_behavior_safety():
    ...
    injection = config.get('prompt_injection', {}) if isinstance(config, dict) else {}
    return jsonify(dict(
        ok=True,
        enabled=safety.get('enabled', True),
        mode='smart_guard',
        checks=['sensitive_terms', 'prompt_injection', 'incoming', 'outgoing', 'context'],
        keyword_count=len(safety.get('blocked_keywords', []) or []),
        prompt_injection_enabled=injection.get('enabled', True),
    ))
...
def api_behavior_safety_toggle():
    try:
        ...
        config.setdefault('prompt_injection', {})['enabled'] = enabled
        save_config(config)
        msg = '智能安全系统已开启' if enabled else '智能安全系统已关闭'
        return jsonify(dict(ok=True, message=msg))
    except Exception as e:
        return jsonify(dict(ok=False, message=str(e))), 400
    ...
    try:
        ...
        if request.method == 'GET':
            return jsonify(ok=True, enabled=injection.get('enabled', True),
                           mode='smart_guard', custom_term_count=len(injection.get('custom_terms', []) or []))
        body = request.get_json(force=True) or {}
        enabled = bool(body.get('enabled', True))
        ...
        return jsonify(ok=True, message='防提示词注入设置已保存', enabled=enabled,
                       custom_term_count=len(clean))
    except Exception as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 400
...
.top-actions{position:fixed;right:18px;top:18px;display:flex;gap:10px;z-index:99}
.theme-btn,.lang-btn{position:static;width:38px;height:38px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:17px;cursor:pointer;transition:transform .2s,box-shadow .2s,border-color .55s cubic-bezier(.4,0,.2,1),background .55s cubic-bezier(.4,0,.2,1),color .55s cubic-bezier(.4,0,.2,1)}
.theme-btn:hover,.lang-btn:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(0,0,0,.12)}
.lang-btn{font-size:15px}
.lang-menu{position:fixed;right:18px;top:64px;z-index:100;background:var(--surface);border:1px solid var(--border);border-radius:10px;box-shadow:var(--shadow);overflow:hidden;min-width:130px;display:none}
.lang-menu button{display:flex;align-items:center;gap:8px;width:100%;padding:10px 14px;border:0;background:transparent;color:var(--text);font-size:13px;cursor:pointer;text-align:left;font-family:inherit}
.lang-menu button:hover{background:rgba(217,119,87,.12)}
.lang-menu button.active{color:#C56545;font-weight:600}
</style>
...
<button class="lang-btn" type="button" onclick="toggleLangMenu(event)" id="langBtn" aria-label="切换语言">文</button>
</div>
<div class="lang-menu" id="langMenu"></div>
<div class="card">
<div class="icon"><i data-lucide="alert-triangle"></i></div>
<h2 id="tTitle">免责声明 / DISCLAIMER</h2>
<p class="sub" id="tSub">请阅读并确认以下声明</p>
<div class="lines" id="tLines">
本项目仅供学习参考，<br>
若因使用本项目产生任何后果，本人一概不负责。
<span class="en" id="tLinesEn">This project is for learning purposes only.<br>Any consequences are solely your own responsibility.</span>
</div>
<div class="inp-row">
...
<script>
var _LANG_TEXTS={
zh:{title:'免责声明 / DISCLAIMER',sub:'请阅读并确认以下声明',lines:'本项目仅供学习参考，\n若因使用本项目产生任何后果，本人一概不负责。',en:'This project is for learning purposes only.\nAny consequences are solely your own responsibility.',ph:'请输入：我同意',confirm:'确认',need:'请输入内容',ok:'\u2713 已确认，跳转中...',fail:'\u2717 请输入"我同意"',reqfail:'请求失败，请重试',aria:'切换语言'},
en:{title:'Disclaimer',sub:'Please read and confirm the following statement',lines:'This project is for learning reference only.\nThe author assumes no responsibility for any consequences arising from its use.',en:'This project is for learning purposes only.\nAny consequences are solely your own responsibility.',ph:'Type: I agree',confirm:'Confirm',need:'Please enter content',ok:'\u2713 Confirmed, redirecting...',fail:'\u2717 Please type "I agree"',reqfail:'Request failed, please retry',aria:'Switch language'},
ru:{title:'Дисклеймер',sub:'Пожалуйста, прочитайте и подтвердите заявление',lines:'Проект предназначен только для учебных целей.\nЗа любые последствия использования автор не несёт ответственности.',en:'This project is for learning purposes only.\nAny consequences are solely your own responsibility.',ph:'Введите: Я согласен',confirm:'Подтвердить',need:'Введите текст',ok:'\u2713 Подтверждено, переход...',fail:'\u2717 Введите «Я согласен»',reqfail:'Ошибка запроса, попробуйте снова',aria:'Сменить язык'}
};
var _LANGS=[{code:'zh',name:'中文'},{code:'en',name:'English'},{code:'ru',name:'Русский'}];
function currentLang(){var l;try{l=localStorage.getItem('panel_lang')}catch(e){l=''}return l||'zh'}
function applyI18n(){var t=_LANG_TEXTS[currentLang()]||_LANG_TEXTS.zh;var b=document.body;document.getElementById('tTitle').textContent=t.title;document.getElementById('tSub').textContent=t.sub;document.getElementById('tLines').childNodes[0].nodeValue=t.lines+'\n';document.getElementById('tLinesEn').textContent=t.en;document.getElementById('agreeInput').placeholder=t.ph;document.getElementById('confirmBtn').textContent=t.confirm;document.getElementById('langBtn').setAttribute('aria-label',t.aria);b.setAttribute('data-lang',currentLang())}
function setLangMenuActive(){var cur=currentLang(),menu=document.getElementById('langMenu');menu.innerHTML=_LANGS.map(function(x){return '<button class="'+(x.code===cur?'active':'')+'" onclick="setLang(\''+x.code+'\')">'+x.name+'</button>'}).join('')}
function toggleLangMenu(e){if(e)e.stopPropagation();var menu=document.getElementById('langMenu');setLangMenuActive();menu.style.display=(menu.style.display==='none')?'block':'none'}
function setLang(code){try{localStorage.setItem('panel_lang',code)}catch(e){}document.getElementById('langMenu').style.display='none';applyI18n()}
document.addEventListener('click',function(e){var m=document.getElementById('langMenu');if(m&&m.style.display==='block'&&!m.contains(e.target)&&e.target.id!=='langBtn')m.style.display='none'});
...
function doConfirm(){
var T=_LANG_TEXTS[currentLang()]||_LANG_TEXTS.zh;
var v=inp.value.trim();
if(!v){msg.textContent=T.need;msg.className='msg err';return}
btn.disabled=true;
fetch('/api/disclaimer/confirm',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({agree:v})})
.then(function(r){return r.json()})
.then(function(d){
if(d.ok){msg.textContent=T.ok;msg.className='msg ok';var next=new URLSearchParams(location.search).get('next')||'/';setTimeout(function(){location.href=next},600)}
else{msg.textContent=T.fail;msg.className='msg err';btn.disabled=false;inp.classList.add('error');setTimeout(function(){inp.classList.remove('error')},400)}
})
.catch(function(){msg.textContent=T.reqfail;msg.className='msg err';btn.disabled=false})
}
</script>
...
.inp-row input::placeholder,.fg input::placeholder,.fg select::placeholder{color:var(--faint)!important}
.card h2{color:var(--text)!important}
.theme-btn{position:fixed;right:18px;top:18px;width:38px;height:38px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:17px;cursor:pointer;z-index:99}
</style>
<script src="https://unpkg.com/lucide@latest/dist/umd/lucide.js"></script>
<script>if(typeof lucide==='undefined'){document.write('<script src="/assets/js/lucide.js"><\/script>')}</script>
</head>
...
.top-actions{position:fixed;right:18px;top:18px;display:flex;gap:10px;z-index:99}
.theme-btn,.lang-btn{width:38px;height:38px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:17px;cursor:pointer;transition:transform .2s,box-shadow .2s,border-color .55s cubic-bezier(.4,0,.2,1),background .55s cubic-bezier(.4,0,.2,1),color .55s cubic-bezier(.4,0,.2,1)}
.theme-btn:hover,.lang-btn:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(0,0,0,.12)}
.lang-btn{font-size:15px;position:relative}
.lang-menu{position:fixed;right:18px;top:64px;z-index:100;background:var(--surface);border:1px solid var(--border);border-radius:10px;box-shadow:var(--shadow);overflow:hidden;min-width:130px;display:none}
.lang-menu button{display:flex;align-items:center;gap:8px;width:100%;padding:10px 14px;border:0;background:transparent;color:var(--text);font-size:13px;cursor:pointer;text-align:left;font-family:inherit}
.lang-menu button:hover{background:rgba(217,119,87,.12)}
.lang-menu button.active{color:#C56545;font-weight:600}
...
<h2 id="tTitle">管理面板登录</h2>
<p class="sub" id="tSub">B站 AI 管理系统</p>
<div class="fg"><label id="tUser">用户名</label><input id="loginUser" type="text" placeholder="输入用户名" autocomplete="off" autofocus></div>
<div class="fg"><label id="tPass">密码</label><input id="loginPass" type="password" placeholder="输入密码" autocomplete="off"></div>
<button class="btn" id="loginBtn" onclick="doLogin()">登录</button>
<div class="msg" id="msg"></div>
<div class="recover"><a href="/forgot-password" id="tRecover">忘记密码</a></div>
</div>
<script>
var _LANG_TEXTS={
zh:{title:'管理面板登录',sub:'B站 AI 管理系统',user:'用户名',spUser:'输入用户名',pass:'密码',spPass:'输入密码',login:'登录',verifying:'验证中...',recover:'忘记密码',need:'请输入用户名和密码',ok:'✓ 登录成功，跳转中...',fail:'✗ ',reqfail:'请求失败，请重试',labels:{aria:'切换语言'}},
en:{title:'Admin Panel Login',sub:'Bilibili AI Management',user:'Username',spUser:'Enter username',pass:'Password',spPass:'Enter password',login:'Sign in',verifying:'Verifying...',recover:'Forgot password',need:'Please enter username and password',ok:'\u2713 Login successful, redirecting...',fail:'\u2717 ',reqfail:'Request failed, please retry',labels:{aria:'Switch language'}},
ru:{title:'Вход в панель',sub:'Bilibili AI Система',user:'Имя пользователя',spUser:'Введите имя',pass:'Пароль',spPass:'Введите пароль',login:'Войти',verifying:'Проверка...',recover:'Забыли пароль',need:'Введите имя и пароль',ok:'\u2713 Успешно, переход...',fail:'\u2717 ',reqfail:'Ошибка запроса',labels:{aria:'Сменить язык'}}
};
var _LANGS=[{code:'zh',name:'中文'},{code:'en',name:'English'},{code:'ru',name:'Русский'}];
function currentLang(){var l;try{l=localStorage.getItem('panel_lang')}catch(e){l=''}return l||'zh'}
function applyI18n(){var t=_LANG_TEXTS[currentLang()]||_LANG_TEXTS.zh;document.getElementById('tTitle').textContent=t.title;document.getElementById('tSub').textContent=t.sub;document.getElementById('tUser').textContent=t.user;document.getElementById('tPass').textContent=t.pass;document.getElementById('loginUser').placeholder=t.spUser;document.getElementById('loginPass').placeholder=t.spPass;document.getElementById('loginBtn').textContent=t.login;document.getElementById('tRecover').textContent=t.recover;document.getElementById('langBtn').setAttribute('aria-label',t.labels.aria)}
function setLangMenuActive(){var cur=currentLang(),menu=document.getElementById('langMenu');menu.innerHTML=_LANGS.map(function(x){return '<button class="'+(x.code===cur?'active':'')+'" onclick="setLang(\''+x.code+'\')">'+x.name+'</button>'}).join('')}
function toggleLangMenu(e){if(e)e.stopPropagation();var menu=document.getElementById('langMenu');setLangMenuActive();menu.style.display=(menu.style.display==='none')?'block':'none'}
function setLang(code){try{localStorage.setItem('panel_lang',code)}catch(e){}document.getElementById('langMenu').style.display='none';applyI18n()}
document.addEventListener('click',function(e){var m=document.getElementById('langMenu');if(m&&m.style.display==='block'&&!m.contains(e.target)&&e.target.id!=='langBtn')m.style.display='none'});
...
var u=inpU.value.trim(),p=inpP.value,T=_LANG_TEXTS[currentLang()]||_LANG_TEXTS.zh;
if(!u||!p){msg.textContent=T.need;msg.className='msg err';return}
...
var d=await r.json();
if(d.ok){msg.textContent=T.ok;msg.className='msg ok';var next=new URLSearchParams(location.search).get('next')||'/';setTimeout(function(){location.href=(d.recovery&&!d.security_question_configured)?'/account-security':next},500)}
else{msg.textContent=T.fail+d.message;msg.className='msg err';btn.disabled=false;btn.textContent=T.login;inpP.value='';inpP.classList.add('error');setTimeout(function(){inpP.classList.remove('error')},400)}
}catch(e){msg.textContent=T.reqfail;msg.className='msg err';btn.disabled=false;btn.textContent=T.login}
}
</script>

<script>if(window.lucide)lucide.createIcons({attrs:{'stroke-width':1.5}});</script></body>
</html>"""
...
_asr_cancel_requested = threading.Event()
...
def _asr_model_dir(asr_cfg: dict) -> Path:
    ...
    # 开源部署：默认模型存放在项目根目录下的 asr 文件夹，随项目一起分发。
    if getattr(sys, 'frozen', False):
        return Path(USER_DATA_DIR) / 'models' / 'asr'
    default = BASE_DIR / 'asr'
    # 兼容旧版本：若新版默认目录为空，但旧目录 model/asr 已有模型，则沿用旧目录避免重复下载。
    legacy = BASE_DIR / 'model' / 'asr'
    if not default.exists() and _asr_has_files(legacy):
        return legacy
    return default


def _asr_has_files(path: Path) -> bool:
...
def api_asr_download():
    ...
    current_status = _asr_status_payload()
    if current_status.get('model_files', 0) and not (request.get_json(silent=True) or {}).get('force'):
        return jsonify(ok=True, already_installed=True, message='已检测到本地模型，不再重复下载', status=current_status)
    ...
    def worker():
        ...
        try:
            ...
            if _asr_cancel_requested.is_set():
                raise InterruptedError('用户已取消 ASR 下载或加载')
            if backend == 'funasr':
            ...
            if _asr_cancel_requested.is_set():
                raise InterruptedError('用户已请求停止；本次模型加载结果未启用')
            files, bytes_ = _asr_disk_usage(model_dir)
...
@app.route('/api/asr/download/stop', methods=['POST'])
def api_asr_download_stop():
    """Request cooperative cancellation of the active model loader."""
    if _asr_download_job.get('state') != 'running':
        return jsonify(ok=True, message='当前没有运行中的 ASR 下载任务', status=_asr_status_payload())
    _asr_cancel_requested.set()
    _asr_download_job.update(phase='stopping', message='已请求停止，正在等待当前模型库调用返回')
    log_line('[ASR] 用户请求停止模型下载或加载')
    return jsonify(ok=True, message='已发送停止请求', status=_asr_status_payload())


@app.route('/api/asr/model/delete', methods=['POST'])
def api_asr_model_delete():
    """Delete only the configured ASR model directory after explicit confirmation."""
    body = request.get_json(silent=True) or {}
    if body.get('confirmed') is not True:
        return jsonify(ok=False, message='请先确认删除本地 ASR 模型'), 400
    if _asr_download_job.get('state') == 'running':
        return jsonify(ok=False, message='请先停止正在运行的 ASR 下载任务'), 409
    cfg = read_json(CONFIG_FILE, {})
    asr_cfg = cfg.get('asr', {}) if isinstance(cfg.get('asr'), dict) else {}
    model_dir = _asr_model_dir(asr_cfg).resolve()
    if len(model_dir.parts) < 3 or model_dir == model_dir.parent or model_dir == Path(model_dir.anchor):
        return jsonify(ok=False, message='模型目录校验失败，拒绝删除'), 400
    try:
        import shutil
        if model_dir.exists():
            shutil.rmtree(model_dir)
        model_dir.mkdir(parents=True, exist_ok=True)
        _asr_download_job.update(state='idle', phase='idle', message='本地模型已删除', finished_at=time.time())
        log_line(f'[ASR] 已删除本地模型目录: {model_dir}')
        return jsonify(ok=True, message='本地 ASR 模型已删除', status=_asr_status_payload())
    except OSError as exc:
        return jsonify(ok=False, message=f'删除 ASR 模型失败: {redact_sensitive_text(str(exc))}'), 500

# ── ASR 测试（自定义链接 / 上传视频 → 测速 + 准确度） ──
_asr_test_job = {'state': 'idle', 'message': '尚未开始', 'started_at': None, 'finished_at': None}
_asr_test_lock = threading.Lock()


def _asr_test_download(url: str, dest: Path) -> Path:
    """下载测试视频到 dest 目录，返回文件路径。支持直链与 B 站链接。"""
    import shutil
    url = url.strip()
    ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0 Safari/537.36"
    # B 站链接 / BV 号 → 用 bilibili_api 下载
    if 'bilibili.com' in url or re.fullmatch(r'BV[0-9A-Za-z]{8,20}', url.split('/')[-1]):
        try:
            from api.client import BiliClient
            client = BiliClient(); client._load_credential()
            from bilibili_api import video as _bili_video
            bvid = url.split('/')[-1].split('?')[0]
            if not re.fullmatch(r'BV[0-9A-Za-z]{8,20}', bvid):
                raise ValueError('无法从链接中解析 BV 号')
            v = _bili_video.Video(bvid=bvid, credential=getattr(client, 'credential', None))
            info = _asyncio.run(v.get_info())
            title = str(info.get('title') or bvid).replace('/', '_').replace('\\', '_')[:60]
            async def _dl():
                from bilibili_api import video as _bv
                download_url_data = await v.get_download_url(0)
                durl = (download_url_data.get('durl') or [{}])[0]
                return durl.get('url') or ''
            video_url = _asyncio.run(_dl())
            if not video_url:
                raise RuntimeError('B 站未返回可下载的视频地址（可能需要登录）')
            out = dest / f"{title}.mp4"
            req = Request(video_url, headers={'User-Agent': ua, 'Referer': 'https://www.bilibili.com/'})
            with urlopen(req, timeout=30) as resp, open(out, 'wb') as fh:
                shutil.copyfileobj(resp, fh)
            return out
        except Exception as exc:
            raise RuntimeError(f'B 站下载失败: {redact_sensitive_text(str(exc))}')
    # 直链
...
def _asr_test_worker(source: str, raw_bytes: bytes | None, filename: str):
    global _asr_test_job
    import tempfile
    tmp = None
    try:
        cfg = read_json(CONFIG_FILE, {})
        ...
        video_path = None
        if raw_bytes:
            name = (filename or 'upload.mp4').replace('/', '_').replace('\\', '_')
            video_path = tmp / (name if name.strip() else 'upload.mp4')
            video_path.write_bytes(raw_bytes)
        elif source:
            _asr_test_job.update(state='running', message='正在下载视频...', started_at=time.time())
            video_path = _asr_test_download(source, tmp)
        else:
            raise ValueError('没有提供视频链接或文件')
        size_mb = video_path.stat().st_size / 1024 / 1024 if video_path.exists() else 0
        _asr_test_job.update(state='running', message='正在提取音频并识别...', started_at=time.time())
        from xingye_bot.asr_engine import ASREngine
        engine = ASREngine(asr_cfg)
        result = _asyncio.run(engine.process_video(video_path, title='ASR测试'))
        t = result.timing or {}
        duration = float(result.duration or 0)
        transcribe_sec = float(t.get('transcribe_seconds') or t.get('total_pipeline_seconds') or 0)
        extract_sec = float(t.get('audio_extract_seconds') or 0)
        total_sec = float(t.get('total_pipeline_seconds') or 0)
        segs = result.segments or []
        confs = [float(getattr(s, 'confidence', 0) or 0) for s in segs if getattr(s, 'confidence', 0)]
        payload = {
            'success': bool(result.success),
            'error': str(result.error or ''),
            'backend': str(result.backend or engine.backend),
            'model_used': str(result.model_used or ''),
            'duration_sec': round(duration, 2),
            'audio_extract_sec': round(extract_sec, 2),
            'transcribe_sec': round(transcribe_sec, 2),
            'total_sec': round(total_sec, 2),
            'real_time_factor': round(transcribe_sec / duration, 3) if duration > 0 else None,
            'file_size_mb': round(size_mb, 2),
            'segments_count': len(segs),
            'text_len': len(str(result.text or '')),
            'avg_confidence': round(sum(confs) / len(confs), 3) if confs else None,
            'speakers_count': len(set(s.speaker for s in segs if s.speaker)),
            'text': str(result.text or ''),
            'segments': [{'t': round(float(s.start), 1), 'd': round(float(s.end or s.start), 1), 'text': s.text, 'sp': s.speaker or ''} for s in segs[:200]],
        }
        _asr_test_job = {'state': 'success', 'result': payload, 'started_at': _asr_test_job.get('started_at'), 'finished_at': time.time(), 'message': '测试完成'}
    except Exception as exc:
        _asr_test_job = {'state': 'failed', 'error': redact_sensitive_text(str(exc))[:500], 'started_at': _asr_test_job.get('started_at'), 'finished_at': time.time(), 'message': '测试失败'}
    finally:
        if tmp is not None:
            try:
...
def api_asr_test():
    from utils.storage import is_safe_path
    if not _asr_test_lock.acquire(blocking=False):
        return jsonify(ok=False, message='已有 ASR 测试正在运行'), 409
    try:
        raw_bytes = None
        filename = ''
        source = ''
        ...
        if raw_bytes is None:
            ...
            if not source:
                _asr_test_job.update(state='idle', message='未提供链接或文件')
                return jsonify(ok=False, message='请提供视频链接或上传视频文件'), 400
        _asr_test_job = {'state': 'running', 'message': '准备中...', 'started_at': time.time(), 'finished_at': None}
        threading.Thread(target=_asr_test_worker, args=(source, raw_bytes, filename), daemon=True).start()
        return jsonify(ok=True, message='ASR 测试已开始', job=dict(_asr_test_job))
    finally:
        _asr_test_lock.release()


@app.route('/api/asr/test/status')
def api_asr_test_status():
    job = dict(_asr_test_job)
    if job.get('started_at') and job.get('state') == 'running':
        job['elapsed_seconds'] = max(0, time.time() - job['started_at'])
...
"""
    ...
    if tray_enabled and tray.start():
        ...
        # 消费桌面通知队列（AI 行为审核、AI 故障等）→ 托盘右下角弹窗
        def _panel_notification_relay():
            from utils.desktop_notifications import take_pending
            while True:
                time.sleep(0.8)
                try:
                    for item in take_pending(DATA_DIR):
                        _tray_now = _system_tray
                        if _tray_now is not None:
                            _tray_now.notify(str(item.get("title") or "BiliLearn"), str(item.get("message") or ""))
                except Exception:
                    pass
        threading.Thread(target=_panel_notification_relay, name="BiliLearnNotifyRelay", daemon=True).start()
        print("[Web] 桌面通知中继已启动", flush=True)
    ...
    try:
        ...
        tray.stop()
        _system_tray = None
@app.route('/api/dm/system/blacklist', methods=['GET', 'POST', 'DELETE'])
def api_dm_system_blacklist():
    """管理私聊黑名单（config.private_message.blacklist_uids）。"""
    cfg = read_json(CONFIG_FILE, {})
    pm = cfg.get("private_message") if isinstance(cfg.get("private_message"), dict) else {}
    blacklist = pm.get("blacklist_uids") or []
    if isinstance(blacklist, str):
        blacklist = [u.strip() for u in blacklist.split(',') if u.strip()]
    blacklist = [str(u) for u in blacklist if str(u).strip()]
    if request.method == 'POST':
        body = request.get_json(silent=True) or {}
        uid = str(body.get("uid") or "").strip()
        if not uid.isdigit():
            return jsonify(ok=False, message="UID 必须是数字"), 400
        if uid not in blacklist:
            blacklist.append(uid)
            pm["blacklist_uids"] = blacklist
            cfg["private_message"] = pm
            write_json(CONFIG_FILE, cfg)
        return jsonify(ok=True, blacklist=blacklist, message="已加入黑名单")
    if request.method == 'DELETE':
        body = request.get_json(silent=True) or {}
        uid = str(body.get("uid") or "").strip()
        if uid in blacklist:
            blacklist = [u for u in blacklist if u != uid]
            pm["blacklist_uids"] = blacklist
            cfg["private_message"] = pm
            write_json(CONFIG_FILE, cfg)
        return jsonify(ok=True, blacklist=blacklist, message="已移出黑名单")
    # GET：补充名字和头像（复用 uid 解析缓存）
    infos = {}
    try:
        infos = _resolve_uid_infos(blacklist)
    except Exception:
        infos = {}
    items = [{
        "uid": uid,
        "name": str((infos.get(uid) or {}).get("name") or "") or f"用户{uid}",
        "avatar": str((infos.get(uid) or {}).get("avatar") or ""),
    } for uid in blacklist]
    return jsonify(ok=True, blacklist=items)


@app.route('/api/dm/system/up-assistant')
def api_dm_system_up_assistant():
    """AI 关注/取关 UP 主的记录。"""
    follow_log = USER_DATA_DIR / "agent_social_follow_log.json"
    records = []
    ...
    for row in records[-100:]:
        ...
        name = ""
        for n, info in ups.items():
            if isinstance(info, dict) and str(info.get("uid")) == uid:
                name = n
                break
        result.append({
            "kind": str(row.get("kind") or "proactive_follow"),
...
@app.route('/api/dm/system/notices')
def api_dm_system_notices():
    """系统通知：最近的机器人运行事件（ALERT / 启动 / 停止 / 提醒）。"""
    events = []
    try:
        log_path = Path(DATA_DIR) / "web_bot_runtime.log"
        if not log_path.exists():
            log_path = Path(DATA_DIR) / "bot_runtime.log"
        if log_path.exists():
            lines = log_path.read_text(encoding='utf-8', errors='replace').splitlines()
            for line in lines[-300:]:
                low = line.lower()
                if any(k in low for k in ("[alert]", "[reminder]", "[alarm]", "[系统]", "启动", "已停止", "退出", "失败", "错误", "[zen]", "ai 错误", "ai error")):
                    events.append({"time": line[:19] if len(line) >= 19 else "", "text": line.strip()[:300]})
    except Exception:
        pass
    return jsonify(ok=True, events=events[-100:])


# UID → 昵称/头像 解析（带内存缓存，避免频繁请求 B 站触发风控）
_uid_info_cache = {}
_uid_info_cache_lock = threading.Lock()

def _resolve_uid_infos(uids):
    """批量解析 UID 的昵称和头像，缓存 30 分钟。查不到返回 用户{uid}。"""
    result = {}
    missing = []
    now = time.time()
    with _uid_info_cache_lock:
        for uid in uids:
            cached = _uid_info_cache.get(uid)
    ...
    if missing:
        ...
        try:
            ...
            client = None
        if client is not None and getattr(client, "credential", None):
            try:
                from bilibili_api import user as _bili_user
                async def _fetch(uid):
                    try:
                        info = await _bili_user.User(int(uid), client.credential).get_user_info()
                        return uid, {"name": str(info.get("name") or ""), "avatar": str(info.get("face") or "")}
                    except Exception:
                        return uid, {"name": "", "avatar": ""}
                async def _fetch_all():
                    return await _asyncio.gather(*[_fetch(u) for u in missing])
                # Flask handles this route in a regular worker thread. Using
                # asyncio.run keeps gather bound to the loop that executes it;
                # the previous manual loop setup could leave _fetch coroutines
                # un-awaited and silently discard every avatar.
                gathered = _asyncio.run(_fetch_all())
                with _uid_info_cache_lock:
                    for uid, info in gathered:
                        _uid_info_cache[uid] = {**info, "ts": time.time()}
                        result[uid] = _uid_info_cache[uid]
            except Exception:
                pass
        # 兜底：私有/凭证失效时，用 B 站公开空间接口解析头像与昵称（无需登录）
        public_fallback = [u for u in missing if u not in result]
        if public_fallback:
            _uid_info_public_fallback(public_fallback, result)
        for uid in missing:
    ...
    for uid in uids:
        info = result.get(uid, {})
        out[uid] = {
            "name": str(info.get("name") or "") or f"用户{uid}",
            "avatar": str(info.get("avatar") or ""),
        }
    return out


def _uid_info_public_fallback(uids, result):
    """使用 B 站公开空间接口（无需登录）解析头像与昵称，写入缓存。"""
    ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0 Safari/537.36"
    for uid in uids:
        try:
            req = Request(
            ...
            info = {
                "name": str(data.get("name") or "") or "",
                "avatar": str(data.get("face") or "") or "",
            }
        except Exception:
            continue
        with _uid_info_cache_lock:
...
def _dm_merged_history(uid: str = "") -> list:
    """合并私信日志与上下文库，保证分区能看到每条收到的私信。

    private_message_log.json 只记录"产生回复/拦截/跳过"的条目（且曾被多进程
    覆盖丢数据）；private_context_db.json 记录所有收到+发送的消息。以
    context_db 为兜底，按 msg_id/时间去重合并，输出统一结构的消息列表。
    """
    merged = []
    seen = set()
    # 1) log 条目（含状态信息：sent/queued/blocked/skipped/reason）
    log_data = read_json(Path(DATA_DIR) / "private_message_log.json", {})
    history = log_data.get("history", []) if isinstance(log_data, dict) else []
    for msg in history:
        if not isinstance(msg, dict):
            continue
        talker = str(msg.get("talker_id") or msg.get("sender_id") or "")
        if uid and talker != str(uid):
            continue
        key = str(msg.get("msg_id") or f"log:{msg.get('timestamp')}:{talker}")
        if key in seen:
            continue
        seen.add(key)
        merged.append({
            ...
            "talker_id": talker,
            "incoming": str(msg.get("incoming") or ""),
            "reply": str(msg.get("reply") or ""),
            "timestamp": str(msg.get("timestamp") or ""),
            "sent": bool(msg.get("sent")),
            "queued": bool(msg.get("queued")),
            "blocked": bool(msg.get("blocked")),
            "skipped": bool(msg.get("skipped")),
            "reason": str(msg.get("reason") or ""),
            "source": "log",
        })
    # 2) context_db 兜底（所有收到/发送的私信渠道消息）
    try:
        ctx = read_json(Path(DATA_DIR) / "private_context_db.json", {})
        for talker, turns in (ctx.items() if isinstance(ctx, dict) else []):
            if talker in ("_memories", "_profiles") or not isinstance(turns, list):
                continue
            if uid and talker != str(uid):
                continue
            for turn in turns:
                ...
                if str(meta.get("channel") or "") != "private_message":
                    continue
                msg_id = str(turn.get("msg_id") or meta.get("msg_id") or "")
                key = msg_id or f"ctx:{turn.get('time')}:{talker}:{turn.get('role')}"
                if msg_id and msg_id in seen:
                    continue
                if msg_id:
                    seen.add(msg_id)
                role = turn.get("role")
                ts = str(turn.get("time") or "")
                sent_flag = True
                if role == "assistant":
                    # 尊重上下文里实际的发送标记；无标记的 assistant 消息默认已发送
                    _meta_sent = meta.get("sent")
                    if _meta_sent is not None:
                        sent_flag = bool(_meta_sent)
                else:
                    sent_flag = False
                merged.append({
                    "msg_id": msg_id,
                    "talker_id": talker,
                    "incoming": str(turn.get("content") or "") if role == "user" else "",
                    "reply": str(turn.get("content") or "") if role == "assistant" else "",
                    "timestamp": ts,
                    "sent": sent_flag,
                    "queued": False,
                    "blocked": False,
...
@app.route('/api/dm/contacts')
def api_dm_contacts():
    try:
        history = _dm_merged_history()
        contacts = {}
        for msg in history:
            if not isinstance(msg, dict): continue
            uid = str(msg.get("talker_id") or msg.get("sender_id") or "")
            if not uid: continue
            if uid not in contacts:
                contacts[uid] = {"uid": uid, "name": f"用户{uid}", "avatar": "", "last_message": "", "last_time": "", "msg_count": 0, "is_self": False}
            contacts[uid]["msg_count"] += 1
            ts = str(msg.get("timestamp") or "")
            incoming = msg.get("incoming") or ""
            reply = msg.get("reply") or ""
            last_txt = reply or incoming
            if msg.get("queued"):
                last_txt = "[待审核] " + (reply or "")
            if msg.get("skipped") and not last_txt:
                last_txt = f"[{msg.get('reason') or '已跳过'}] " + (incoming or "")
            if ts > contacts[uid]["last_time"]:
                contacts[uid]["last_time"] = ts
                contacts[uid]["last_message"] = last_txt[:80]
        contact_list = sorted(contacts.values(), key=lambda x: x["last_time"], reverse=True)
        uids = [c["uid"] for c in contact_list]
        infos = {}
        try:
            infos = _resolve_uid_infos(uids)
        except Exception:
            infos = {}
        for c in contact_list:
            info = infos.get(c["uid"], {})
            c["name"] = str(info.get("name") or "") or c["name"]
            c["avatar"] = str(info.get("avatar") or "")
        return jsonify(ok=True, contacts=contact_list)
    except Exception as e:
        return jsonify(ok=False, message=str(e)), 500


@app.route('/api/dm/history/<uid>')
def api_dm_history(uid):
    try:
        history = _dm_merged_history(uid)
        messages = []
        for msg in history:
            if not isinstance(msg, dict): continue
            talker = str(msg.get("talker_id") or "")
            incoming = msg.get("incoming") or ""
            reply = msg.get("reply") or ""
            ts = str(msg.get("timestamp") or "")
            sent = bool(msg.get("sent"))
            ...
            # 展示用户发来的
            if incoming:
                messages.append({"role": "user", "content": incoming, "time": ts, "uid": talker})
            # 展示 AI 回复（明确区分审核状态，避免误导）
            if reply:
                if sent:
                    status = "已发送"
                elif queued:
                    status = "待审核"  # 进了AI行为审核，尚未真正发送
                else:
                    status = "未发送"
                messages.append({"role": "ai", "content": reply, "time": ts, "uid": "", "status": status})
            # 展示跳过/拦截的条目（只有用户消息，AI未回复）
            elif skipped or blocked:
                status = "已跳过" if skipped else "已拦截"
                reason = str(msg.get("reason") or "")
                if reason:
                    status += f"（{reason}）"
                messages.append({"role": "ai", "content": "", "time": ts, "uid": "", "status": status})
        self_uid = ""
        self_name = "AI"
        self_avatar = ""
        try:
            cookie_data = read_json(Path(DATA_DIR) / "bilibili_cookies.json", {})
            self_uid = str(cookie_data.get("DedeUserID") or cookie_data.get("uid") or "")
        except Exception:
            pass
        peer_info = {}
        self_info = {}
        try:
            infos = _resolve_uid_infos(list(dict.fromkeys([uid] + ([self_uid] if self_uid else []))))
            peer_info = infos.get(uid, {}) or {}
            self_info = infos.get(self_uid, {}) if self_uid else {}
        except Exception:
            peer_info, self_info = {}, {}
        return jsonify(
            ok=True, history=messages, uid=uid, self_uid=self_uid,
            peer_name=str(peer_info.get("name") or "") or f"用户{uid}",
            peer_avatar=str(peer_info.get("avatar") or ""),
            self_name=str(self_info.get("name") or "AI"),
            self_avatar=str(self_info.get("avatar") or ""),
        )
    except Exception as e:
        return jsonify(ok=False, message=str(e)), 500
...
def api_export_multi():
    try:
        ...
        result = export_text(body.get("content", ""), body.get("filename", "export"), body.get("type", "txt"))
...
def api_update_skip():
    """用户跳过当前提示的新版本（记住该版本，之后不再自动提醒）。"""
    try:
        body = request.get_json(force=True) or {}
        version = str(body.get('version') or body.get('latest_version') or '').strip()
        if not version:
            return jsonify(ok=False, message='缺少版本号'), 400
        _write_skipped_version(version)
        log_line(f'[UPDATE] 用户跳过了版本 {version}')
        return jsonify(ok=True, message=f'已跳过版本 {version}，之后不再自动提醒')
    except Exception as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 500
...
def api_update_unskip():
    ...
    try:
        ...
        log_line('[UPDATE] 用户取消了跳过版本')
        return jsonify(ok=True, message='已重新启用更新提醒')
    except Exception as exc:
        return jsonify(ok=False, message=redact_sensitive_text(str(exc))), 500
