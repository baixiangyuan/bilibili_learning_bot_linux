...
def test_safety_api_exposes_plaintext_and_encrypted_storage_views(tmp_path, monkeypatch):
    ...
    with web_panel.app.test_client() as client:
        ...
        # 新设计：关键词规则收归后端，接口只暴露启用开关与词库计数，不泄露具体词条
        assert shown["enabled"] is True
        assert shown["keyword_count"] == 1
        assert "keywords" not in shown
        assert "encrypted_keywords" not in shown

        # 保存接口仍可用（plaintext 视图保存），但不返回明文关键词
        saved = client.post("/api/behavior/safety/save", json={
            "view": "plaintext", "keywords": ["test-term", "another"],
        })
